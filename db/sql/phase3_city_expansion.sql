...
values
  ...
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-pinheiros-hub','Pinheiros Hub',-23.5667,-46.7000,'Pinheiros','hub','active-pressure','Active hub, retail drag, and no reward for soft timing.','Clock the side street riders actually trust in the middle of the rush.','Touch the hub and lock the hard exit instantly.','Give the canyon one fake dispatch line and disappear.',70,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-butanta-seam','Butantã Seam',-23.5750,-46.7083,'Butantã','seam','uni-drag','University edge, river pull nearby, and no help if you get soft.','Clock the exact point the water stops helping and starts baiting you.','Touch the seam, pick the quick exit, and don’t overthink it.','Throw one fake champion nod at the campus and bounce.',80,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-bom-retiro-cut','Bom Retiro Cut',-23.5250,-46.6333,'Bom Retiro','street-cut','commerce-pressure','Textile block, heavy air, and long blocks that punish lazy reads.','Clock the corner that feels more neighborhood than performance.','Touch the cut, read the exit quick, and keep the cadence mean.','Give the block one fake sponsor tag and disappear.',90,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-pompeia-hill','Pompeia Hill',-23.5333,-46.6833,'Pompeia','rise','hill-bite','Hill bite, bar scene nearby, and no room for soft timing.','Notice where the street stops feeling flat and starts feeling personal.','Tag the rise, breathe once, and push through before it turns into a thing.','Make one fake heroic face at the slope and keep it pushing.',100,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-vila-mariana-line','Vila Mariana Line',-23.5833,-46.6333,'Vila Mariana','line','dense-read','Dense residential, wide reads, and no patience for bad reads.','Clock the side street that feels cleaner than the boulevard deserves.','Touch the line, lock the exit, and keep the move blunt.','Make one fake radio-check to yourself and carry on.',110,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-santa-cecilia-grit','Santa Cecilia Grit',-23.5417,-46.6500,'Santa Cecilia','pocket','old-core','Old-core grit, rail drag nearby, and cleaner seams than they look.','Clock the least polished useful corner and trust it more than the obvious one.','Touch the grit, keep the pace honest, and leave before it drags.','Quietly rate the whole block like a harsh race judge and move on.',120,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-itaim-bibi-canyon','Itaim Bibi Canyon',-23.5833,-46.6833,'Itaim Bibi','canyon','tower-pressure','Tower pressure, canyon wind, and no reward for soft timing.','Clock the block that feels more messenger than office spill.','Touch the canyon and keep the move neat, not polite.','Mutter one fake team-director call to yourself and carry on.',130,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-jardins-trap','Jardins Trap',-23.5667,-46.6667,'Jardins','grid','pretty-trap','Pretty trap, high-end grid, and better cuts hiding off the clean pull.','Find the least polished useful corner and trust it.','Touch the trap and get out before the block tries to keep your attention.','Give the block a fake style award and vanish.',140,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-vila-maria-seam','Vila Maria Seam',-23.5167,-46.5833,'Vila Maria','seam','industrial-drag','Border drag, warehouse edges, and no reward for soft timing.','Clock the side street that feels more rider than truck spill.','Touch the seam, pick the hard exit, and keep the pace blunt.','Give the block one fake dispatch line and move on.',150,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-jacana-pocket','Jaçanã Pocket',-23.4500,-46.5833,'Jaçanã','pocket','neighborhood-pressure','Neighborhood rhythm, old-core pressure, and one clean side.','Find the corner that looks calm but clearly never is.','Touch the pocket and leave before it flattens you.','Act like this was your secret lane and keep moving.',160,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-tucuruvi-rise','Tucuruvi Rise',-23.4750,-46.6000,'Tucuruvi','rise','hill-bite','Hill bite, transit pressure, and no help if you get soft.','Clock how fast the altitude changes the block mood.','Tag the rise, breathe once, and punch through clean.','Mutter one fake team-radio line and disappear.',170,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-santana-grid','Santana Grid',-23.5000,-46.6167,'Santana','grid','active-drag','Active center, bus pull, and cleaner lines just off the main drag.','Clock the cross street that feels worth trusting here.','Touch the grid, lock the exit, and keep your pace honest.','Throw one fake champion glare at the flyover and bounce.',180,true),
  ((select id from pack_refs where slug = 'saopaulo'),'saopaulo-parque-novo-mundo-cut','Novo Mundo Cut',-23.5000,-46.5667,'Parque Novo Mundo','street-cut','industrial-seam','Industrial seam, border pressure, and no reward for drifting.','Clock the first corner that feels more worker than visitor.','Touch the cut and commit to the hard exit instantly.','Rate the block like a harsh race judge and vanish.',190,true)
on conflict (slug) do update
set
