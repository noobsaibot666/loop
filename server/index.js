...
import {
  ...
  getQuarterWindow,
  isInWindow,
} from '../shared/quarterly.js';
import {
  buildAlleycatHistory,
...
} from '../shared/community-events.js';
...
const sanitizeBikePayload = (input = {}) => ({
  ...
  bike_name:
    String(input?.bike_name || '')
      .trim()
      .slice(0, 60) || null,
  bike_ratio:
    String(input?.bike_ratio || '')
      .trim()
      .slice(0, 40) || null,
});
...
const getCommunityRecipient = async userId => {
  ...
  const authResult = await supabase.auth.admin
    .getUserById(userId)
  ...
  return {
    ...
    riderName:
      profile?.rider_name?.trim() || authUser?.email?.split('@')[0] || 'Rider',
  };
};
...
const buildManifestFromDatabasePack = ({
  pack,
  checkpoints,
  ghostEnabled = true,
  difficulty,
...
const consumeNightRideCredit = async (authUser, sessionType = 'single') => {
  ...
  if ((sessionType === 'single' || sessionType === 'join') && free_used < 3) {
    ...
    return {
      ...
      error:
        sessionType === 'crew'
          ? 'Crew Night Ride needs 2 credits.'
          : sessionType === 'join'
            ? 'Joining a crew Night Ride needs 1 credit or a free loop left.'
            : 'Night Ride needs 1 credit or a free loop left.',
    };
  }
  const {error: updateError} = await supabase
    .from('user_credits')
    .upsert(
      {user_id: authUser.id, free_used, credits: credits_remaining},
      {onConflict: 'user_id'},
    );
  if (updateError) throw updateError;
  return {
  ...
  async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const isMembership =
        ...
        session.metadata?.plan_code === 'discord_access';
        ...
        await recordCommunityEvent(process.env, {
          ...
          details: {
            ...
            stripe_subscription_id:
              nextMembership.stripe_subscription_id || null,
          },
        }).catch(() => null);
      ...
      if (user_id) {
        ...
        await supabase
          .from('user_credits')
          .upsert(
            {user_id, credits: currentCredits + creditAdd},
            {onConflict: 'user_id'},
          );

        await safeNoThrow(
          supabase.from('stripe_sessions').upsert(
            {
        ...
        let nextMembership = {
          ...
          ...buildMembershipUpsert({
            ...
            checkoutSession: {
              ...
              metadata: {
                plan_code:
                  subscription?.metadata?.plan_code || COMMUNITY_PLAN_CODE,
                discord_invite_url:
...
app.post('/api/usage/check', async (req, res) => {
  ...
  if (!device_id && !user_id)
    return res.status(400).json({error: 'device_id or user_id required'});
  ...
  if (user_id) {
    ...
    return res.json({
      ...
      free_remaining: isAdmin
        ? 9999
        : Math.max(0, FREE_LIMIT - usage.free_used),
...
const recordModerationAction = async ({
  ...
  await safeNoThrow(
    supabase.from('moderation_action_history').insert({
      ...
      .map(authUser => {
        ...
        return {
          ...
          updated_at:
            ...
            null,
          bikes: bikesGrouped.get(authUser.id) || [],
        };
      })
      .sort((left, right) =>
...
app.post('/api/admin/set-credits', requireAdmin, async (req, res) => {
  ...
  if (user_id) {
    const {error} = await supabase
      .from('user_credits')
      .upsert(
        {user_id, free_used, credits: credits || donation_credits},
        {onConflict: 'user_id'},
      );
    if (error) return res.status(500).json({error: error.message});
    return res.json({
      ...
      user_id,
      free_used,
      credits: credits || donation_credits,
    });
  }
  const {error} = await supabase
    .from('device_usage')
    .upsert(
      {device_id, free_used, donation_credits},
...
app.post('/api/admin/overview', requireAdmin, async (req, res) => {
  ...
  const [
    ...
    supabase
      .from('stripe_sessions')
      .select('session_id, status, amount_cents')
      .order('created_at', {ascending: false})
      .limit(5),
    supabase
      .from('messenger_manifests')
      .select('id, city_name, manifest_title, checkpoint_count, ghost_seconds'),
    supabase
      .from('messenger_runs')
      .select(
        'id, user_id, manifest_id, status, finish_seconds, finished_at, bike_name, bike_ratio',
      ),
    supabase.from('messenger_challenges').select('id'),
    supabase
      .from('messenger_proof_posts')
      .select(
        'id, rider_name, city_name, checkpoint_name, is_public, created_at, public_url, storage_path',
      )
      .order('created_at', {ascending: false})
      .limit(12),
    supabase
  ...
  const fastestRunCandidates = (runs || [])
    .filter(
      run =>
        run.status === 'finished' &&
        typeof run.finish_seconds === 'number' &&
        manifestMap.has(run.manifest_id),
    )
    .sort((left, right) => left.finish_seconds - right.finish_seconds)
    .slice(0, 12);
  const fastestUserIds = [
    ...new Set(fastestRunCandidates.map(run => run.user_id).filter(Boolean)),
  ];
  const {data: profileRows} = fastestUserIds.length
    ? await supabase
        .from('user_profiles')
        .select('user_id, rider_name')
        .in('user_id', fastestUserIds)
    : {data: []};
  const profileMap = new Map(
    (profileRows || []).map(profile => [profile.user_id, profile.rider_name]),
  );
...
app.post('/api/admin/community-retry', requireAdmin, async (req, res) => {
  ...
  if (!event.user_id)
    return res.status(400).json({error: 'event has no user_id'});
  ...
  try {
    ...
    return res
      .status(500)
      .json({error: error instanceof Error ? error.message : 'Retry failed'});
  }
});
...
app.post('/api/admin/night-rides', requireAdmin, async (_req, res) => {
  const {data, error} = await supabase
    ...
    .select(
      'id, user_id, rider_name, crew_name, city_name, route_title, distance_km, caption, image_url, aspect_ratio, is_public, moderation_status, created_at',
    )
    .order('created_at', {ascending: false})
    .limit(48);
  if (error) return res.status(500).json({error: error.message});
...
app.post('/api/admin/night-ride-moderation', requireAdmin, async (req, res) => {
  ...
  const moderation_status = String(req.body?.moderation_status || '')
    .trim()
    .toLowerCase();
  ...
  const {data: existing, error: existingError} = await supabase
    .from('night_ride_posts')
    .select(
      'id, rider_name, crew_name, city_name, route_title, moderation_status, is_public',
    )
    .eq('id', post_id)
    .maybeSingle();
  if (existingError)
  ...
  const nextPublic =
    moderation_status === 'hidden' ? false : existing.is_public !== false;
  const {data, error} = await supabase
    .from('night_ride_posts')
    .update({
      moderation_status,
...
app.post('/api/admin/city-packs', requireAdmin, async (req, res) => {
  const getPackReadiness = (pack, countData, districtsByPack) => {
    ...
    const copy_ready = Boolean(
      pack.route_note && pack.finish_label && pack.safety_note,
    );
    const can_publish =
      copy_ready && active_checkpoint_count >= 4 && district_count >= 3;
  ...
  if (packsRes.error)
    return res.status(500).json({error: packsRes.error.message});
  if (checkpointsRes.error)
    return res.status(500).json({error: checkpointsRes.error.message});
  const counts = new Map();
  const districtsByPack = new Map();
  for (const checkpoint of checkpointsRes.data || []) {
    const current = counts.get(checkpoint.pack_id) || {
      checkpoint_count: 0,
...
app.post('/api/admin/city-checkpoints', requireAdmin, async (req, res) => {
  ...
  if (req.body?.action === 'save') {
    ...
    if (
      ...
      return res
        .status(400)
        .json({error: 'pack_id, slug, name, lat, and lng required'});
    }
    if (
      !payload.hint ||
      !payload.task_local ||
      !payload.task_fast ||
      !payload.task_chaotic
    ) {
      return res
        .status(400)
        .json({error: 'hint and all task variants required'});
    }
    const {data, error} = await supabase
      .from('city_checkpoints')
      .upsert(payload, {onConflict: 'slug'})
...
app.post('/api/admin/preview-manifest', requireAdmin, async (req, res) => {
  const difficulty = String(req.body?.difficulty || 'medium')
    .trim()
    .toLowerCase();
  const style = String(req.body?.style || 'local')
    .trim()
    .toLowerCase();
  const seed = Number(req.body?.seed || 777);
  const checkpointCount = Number(req.body?.checkpoint_count || 0) || null;
  const startPoint =
    Number.isFinite(Number(req.body?.start_lat)) &&
    Number.isFinite(Number(req.body?.start_lng))
  ...
  return res.json({
    ...
    source: packId || city ? 'database-or-fallback' : 'fallback',
  });
});
...
app.post('/api/admin/proof-visibility', requireAdmin, async (req, res) => {
  ...
  const {data, error} = await supabase
    ...
    .eq('id', proof_id)
    .select(
      'id, rider_name, city_name, checkpoint_name, is_public, created_at, public_url',
    )
    .limit(1);
...
app.post('/api/admin/proof-delete', requireAdmin, async (req, res) => {
  ...
  const {data: proof, error: proofError} = await supabase
    .from('messenger_proof_posts')
    .select(
      'id, storage_path, rider_name, city_name, checkpoint_name, is_public, archived_at',
    )
    .eq('id', proof_id)
    .maybeSingle();
  ...
  if (proof.storage_path) {
    await supabase
      ...
      .delete()
      .eq('bucket_id', 'alleycat-proofs')
      .eq('name', proof.storage_path);
  }
...
app.post('/api/admin/city-requests', requireAdmin, async (req, res) => {
  if (String(req.body?.action || '') === 'ai_draft') {
    ...
    const cityName = titleCaseCity(
      requestRow.requested_city || requestRow.requested_location || '',
    );
    const slug = slugifyCity(cityName);
    if (!cityName || !slug)
      return res
        .status(400)
        .json({error: 'request needs a city name before drafting'});
...
app.post('/api/admin/collaborations', requireAdmin, async (req, res) => {
  if (String(req.body?.action || '') === 'update') {
    ...
    const collaborationStatus =
      String(req.body?.collaboration_status || '').trim() || 'pending';
    ...
    const {data, error} = await supabase
      ...
      .eq('user_id', userId)
      .select(
        'user_id, rider_name, home_location, collaboration_note, collaboration_status, collaboration_requested_at, updated_at',
      )
      .limit(1);
...
app.post('/api/account/summary', async (req, res) => {
  ...
  const [
    ...
    (async () => {
      try {
        return await supabase
          .from('user_profiles')
          .select(
            'user_id, rider_name, home_location, bike_name, bike_ratio, primary_bike_id, collaboration_note, collaboration_status, collaboration_requested_at',
          )
          .eq('user_id', user_id)
          .maybeSingle();
      } catch {
    ...
    supabase
      .from('loop_history')
      .select(
        'id, loop_point, distance_km, unit, terrain, surface, vibe, route_url, created_at',
      )
      .eq('user_id', user_id)
      .order('created_at', {ascending: false})
      .limit(8),
    supabase
      .from('messenger_manifests')
      .select(
        'id, city_name, manifest_title, difficulty, style, created_at, source_challenge_id, ghost_seconds',
      )
      .eq('user_id', user_id),
    supabase
      .from('messenger_runs')
      .select(
        'id, user_id, manifest_id, status, finish_seconds, finished_at, started_at',
      )
      .eq('user_id', user_id),
    supabase
    ...
    supabase
      .from('messenger_proof_posts')
      .select(
        'id, user_id, run_id, is_public, city_name, manifest_id, checkpoint_id, checkpoint_name, location_label, public_url, created_at',
      )
      .eq('user_id', user_id),
    supabase
      .from('messenger_proof_posts')
    ...
    (async () => {
      try {
        return await supabase
          .from('community_memberships')
          .select(
            'user_id, plan_code, status, price_cents, currency, interval, current_period_end, cancel_at_period_end, discord_invite_url, discord_user_id, discord_username, discord_role_status, discord_access_granted_at, discord_access_revoked_at',
          )
          .eq('user_id', user_id)
          .maybeSingle();
      } catch {
  ...
  const challengeIds = [
    ...new Set(userChallengeEntries.map(entry => entry.challenge_id)),
  ];
  const [
    userChallenges,
    challengeEntriesByChallenge,
    challengeManifests,
    challengeRuns,
    challengeProofNames,
  ] = await Promise.all([
      ...
      ? Promise.all(
          challengeIds.map(async challengeId => {
            const {data} = await supabase
              ...
              .select(
                'id, code, creator_user_id, claimed_by_user_id, created_at',
              )
              .eq('id', challengeId)
              .limit(1);
            return data?.[0] || null;
      ...
      ? Promise.all(
          challengeIds.map(async challengeId => {
            const {data: entriesData} = await supabase
              ...
              .select('manifest_id')
              .eq('challenge_id', challengeId);
            const manifestIds = (entriesData || []).map(
  ...
  const userQuarterRuns = userRuns.filter(
    run =>
      run.status === 'finished' &&
      isInWindow(run.finished_at, quarter.start, quarter.end),
  );
  const userQuarterProofs = userProofs.filter(
    proof =>
      proof.is_public &&
      isInWindow(proof.created_at, quarter.start, quarter.end),
  );
  const challengeManifestIds = new Set(
    userManifests
      .filter(manifest => manifest.source_challenge_id)
      .map(manifest => manifest.id),
  );
  const userQuarterRank =
    quarterLeaderboard.find(entry => entry.user_id === user_id)?.rank || null;
  ...
  return res.json({
    ...
    bikes: bikes || [],
    purchases: purchases || [],
    community_membership: sanitizeMembershipForClient(
    ...
    alleycat_history: buildAlleycatHistory({
      manifests: [
        ...userManifests,
        ...challengeManifests.filter(
          item => !userManifests.find(own => own.id === item.id),
        ),
      ],
      runs: [
        ...userRuns,
        ...challengeRuns.filter(
          item => !userRuns.find(own => own.id === item.id),
        ),
      ],
      proofs: userProofs,
    }),
...
app.post('/api/account/profile', async (req, res) => {
  ...
  let bikesToUpsert = inputBikes
    ...
    .map((b, i) => {
      const bikeObj = {
        user_id,
        bike_name: String(b.bike_name || '')
          .trim()
          .slice(0, 60),
        bike_ratio: String(b.bike_ratio || '')
          .trim()
          .slice(0, 40),
  ...
  if (!bikesToUpsert.length && (req.body?.bike_name || req.body?.bike_ratio)) {
    bikesToUpsert = [
      {
        user_id,
        bike_name: String(req.body?.bike_name || '')
          .trim()
          .slice(0, 60),
        bike_ratio: String(req.body?.bike_ratio || '')
          .trim()
          .slice(0, 40),
        is_default: true,
        sort_order: 0,
  ...
  if (bikesToUpsert.length) {
    ...
    if (upsertError) {
      ...
      return res
        .status(500)
        .json({error: 'Failed to save bikes: ' + upsertError.message});
    }
    if (upsertedBikes) {
  ...
  const primaryBike =
    finalBikes.find(b => b.is_default) || finalBikes[0] || null;

  const payload = {
    user_id,
    rider_name: String(req.body?.rider_name || '')
      .trim()
      .slice(0, 40),
    home_location: String(req.body?.home_location || '')
      .trim()
      .slice(0, 120),
    bike_name: primaryBike?.bike_name || null,
    bike_ratio: primaryBike?.bike_ratio || null,
    primary_bike_id: primaryBike?.id || null,
    collaboration_note: String(req.body?.collaboration_note || '')
      .trim()
      .slice(0, 600),
    collaboration_status: submitCollaboration
      ? 'pending'
      : String(req.body?.collaboration_status || '')
          .trim()
          .slice(0, 20) || null,
    collaboration_requested_at: submitCollaboration
      ? new Date().toISOString()
      : req.body?.collaboration_requested_at || null,
    updated_at: new Date().toISOString(),
  };
...
app.post('/api/account-feedback', async (req, res) => {
  ...
  const trimFeedback = (value = '') =>
    String(value).replace(/\s+/g, ' ').trim();
  const countWords = (value = '') =>
    (trimFeedback(value).match(/\S+/g) || []).length;
  ...
  if (feedback.length > CHAR_LIMIT)
    return res
      .status(400)
      .json({error: `keep it under ${CHAR_LIMIT} characters`});
  if (countWords(feedback) > WORD_LIMIT)
    return res.status(400).json({error: `keep it under ${WORD_LIMIT} words`});
  ...
  if (error) {
    if (
      String(error.message || '')
        .toLowerCase()
        .includes('account_feedback')
    ) {
      return res.status(500).json({
        error:
          'Feedback table is not ready in Supabase yet. Apply account_feedback.sql first.',
      });
    }
    return res.status(500).json({error: error.message});
  }
...
app.post('/api/usage/consume', async (req, res) => {
  ...
  if (!allowed) {
    return res.json({
      ...
      free_used,
      donation_credits,
      credits_remaining: donation_credits,
    });
  }
...
app.post('/api/save-setup', async (req, res) => {
  const {device_id, loop_point, distance, unit, terrain, surface, vibe} =
    req.body || {};
  const authUser = await getAuthUser(req);
...
app.post('/api/create-checkout-session', async (req, res) => {
  ...
  const successRedirectTo = sanitizeRedirectUrl(
    req.body?.success_redirect_to,
    `${APP_URL}/account`,
  );
  const cancelRedirectTo = sanitizeRedirectUrl(
    req.body?.cancel_redirect_to,
    `${APP_URL}/account`,
  );
    ...
    line_items: [
      {
        price_data: {
          ...
          product_data: {
            ...
            description:
              'Discord community access with optional bonus credits later.',
          },
...
app.post('/api/stripe/verify-session', async (req, res) => {
  ...
  const creditAdd =
    existingSession?.credits_to_grant || creditsFromAmount(amount);
  const {data} = await supabase
    .from('user_credits')
  ...
  const membership = buildMembershipUpsert({
    ...
    checkoutSession: {
      ...
      metadata: {
        ...
        discord_invite_url:
          session.metadata?.discord_invite_url || COMMUNITY_INVITE_URL,
      },
    },
    subscription,
  });
...
app.post('/api/community-membership/discord-start', async (req, res) => {
  ...
  if (!isMembershipActive(membership)) {
    return res
      .status(403)
      .json({error: 'membership inactive', access_state: 'inactive'});
  }
  ...
  const config = getDiscordConfig(process.env, {
    url: `${APP_URL}/api/community-membership/discord-start`,
  });
  await safeNoThrow(
    supabase.from('community_memberships').upsert(
      {
...
app.get('/api/community-membership/discord-callback', async (req, res) => {
  ...
  const redirectBase = sanitizeRedirectUrl(
    decodeRedirectState(state),
    `${APP_URL}/account`,
  );
  const redirect = outcome =>
    res.redirect(appendRedirectParams(redirectBase, {community: outcome}));
  ...
  try {
    const config = getDiscordConfig(process.env, {
      url: `${APP_URL}/api/community-membership/discord-callback`,
    });
      ...
      supabase.from('community_memberships').upsert(
        {
          ...
          discord_linked_at:
            membership.discord_linked_at || new Date().toISOString(),
          ...
          discord_last_error: null,
          discord_invite_url:
            membership.discord_invite_url || COMMUNITY_INVITE_URL,
        },
        {onConflict: 'user_id'},
      ),
    );
...
app.post('/api/community-membership/access', async (req, res) => {
  ...
  if (!membership)
    return res
      .status(404)
      .json({error: 'membership not found', access_state: 'inactive'});
  ...
  if (membership.stripe_subscription_id) {
    try {
      ...
      currentMembership = {
        ...membership,
        stripe_customer_id:
          subscription.customer || membership.stripe_customer_id || null,
        ...
        discord_invite_url:
          subscription?.metadata?.discord_invite_url ||
          membership.discord_invite_url ||
          COMMUNITY_INVITE_URL,
      };
      await safeNoThrow(
        supabase
          .from('community_memberships')
          .upsert(currentMembership, {onConflict: 'user_id'}),
      );
    } catch {}
  }
  ...
  if (!isMembershipActive(currentMembership)) {
    return res
      .status(403)
      .json({error: 'membership inactive', access_state: accessState});
  }
  if (
    !currentMembership.discord_user_id ||
    currentMembership.discord_role_status === 'link_required'
  ) {
...
app.post('/api/stripe/portal', async (req, res) => {
  ...
  if (!customerId)
    return res.status(400).json({error: 'customer portal unavailable'});
...
app.post('/api/loop', async (req, res) => {
  ...
  const recentRoutes = authUser?.id
    ? (
        await safeNoThrow(
          supabase
            .from('loop_history')
...
app.post(
  ['/api/night-rides/create', '/api/night-rides/generate'],
  async (req, res) => {
    const authUser = await getAuthUser(req);
    const user_id = authUser?.id || '';
    if (!user_id) return res.status(401).json({error: 'login required'});
    ...
    const crew_name = String(req.body?.crew_name || '')
      .trim()
      .slice(0, 80);
    const ride_city = String(req.body?.ride_city || '')
      .trim()
      .slice(0, 80);
    const crew_members = sanitizeCrewMembers(req.body?.crew_members);
    const requestedBike = sanitizeBikePayload(req.body);
    const origin = {
      lat: Number(req.body?.origin_lat),
      lng: Number(req.body?.origin_lng),
    };
    const destination =
      mode === 'roulette'
        ? {
    ...
    if (mode === 'loop') {
      ...
      if (!response.ok)
        return res
          .status(response.status)
          .json({error: data?.error?.message || data?.message || 'ORS error'});
      route_payload = data;
      const coords = data?.features?.[0]?.geometry?.coordinates || [];
      const waypoints = sampleLoopWaypoints(coords, distance_km);
      const resolvedWaypoints = hasUsableLoopWaypoints(waypoints)
        ? waypoints
        : buildNightRideFallbackLoopWaypoints(
            origin,
            distance_km,
      ...
      route_url = buildNightRideMapsUrl({
        origin,
        destination: origin,
      ...
      title =
        session_type === 'crew'
          ? `${crew_name} · Night Loop`
          : `Night Loop · ${origin_label}`;
    } else {
      const via = buildRouletteWaypoint({
        start: origin,
        end: destination,
        targetKm: distance_km,
        difficulty,
      });
      const response = await fetch(
        'https://api.openrouteservice.org/v2/directions/cycling-regular',
    ...
    if (session?.id) {
      await safeNoThrow(
        supabase.from('night_ride_participants').insert({
          session_id: session.id,
          user_id,
          rider_name:
            profile?.rider_name?.trim() ||
          ...
          joined_via: 'creator',
          credits_spent:
            session_type === 'crew' ? NIGHT_RIDE_CREW_BUILD_COST : 1,
        }),
      );
    }
...
app.post('/api/night-rides/join', async (req, res) => {
  ...
  const code = String(req.body?.code || req.body?.share_code || '')
    .trim()
    .toUpperCase();
  ...
  if (error) return res.status(500).json({error: error.message});
  if (!session)
    return res.status(404).json({error: 'night ride code not found'});
  if (session.status !== 'open')
    return res.status(409).json({error: 'night ride is closed'});
  if (session.session_type !== 'crew')
    return res
      .status(409)
      .json({error: 'single night rides do not take join codes'});
...
app.post('/api/night-rides/post', async (req, res) => {
  ...
  const aspectRatio =
    String(req.body?.aspect_ratio || '1:1').trim() === '16:9' ? '16:9' : '1:1';
  ...
  if (!sessionId || !imageUrl || !storagePath) {
    return res
      .status(400)
      .json({error: 'session_id, image_url, and storage_path required'});
  }
  ...
  const {data: session} = await supabase
    ...
    .maybeSingle();
  if (!session)
    return res.status(404).json({error: 'night ride session not found'});
  ...
  const {data: profile} = await supabase
    ...
    .maybeSingle();
  const riderName =
    profile?.rider_name?.trim() || riderLabelFromEmail(authUser?.email || '');
...
app.get('/api/night-rides/mine', async (req, res) => {
  ...
  const {data: owned, error} = await supabase
    .from('night_ride_sessions')
    .select(
      'id, title, session_type, mode, difficulty, distance_km, ride_city, crew_name, crew_members, bike_id, bike_name, bike_ratio, created_at',
    )
  ...
  const joinedIds = Array.from(
    new Set((joined || []).map(row => row.session_id).filter(Boolean)),
  );
  let joinedSessions = [];
  if (joinedIds.length) {
    const {data} = await supabase
...
app.post('/api/messenger/generate', async (req, res) => {
  ...
  const ghostEnabled =
    req.body?.ghost_enabled !== false &&
    req.body?.ghost_enabled !== 'false' &&
    req.body?.ghost_enabled !== 0;
  if (
    !String(start_label || '').trim() ||
    !Number.isFinite(Number(start_lat)) ||
    !Number.isFinite(Number(start_lng))
  ) {
  ...
  const dbCheckpoints = dbPack
    ? await getDbPackCheckpoints(dbPack.id, true)
    : [];
  const dbBuilt =
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Messenger generation failed';
...
app.post('/api/messenger/start', async (req, res) => {
  ...
  if (manifestError)
    return res.status(500).json({error: manifestError.message});
...
app.post('/api/messenger/restart', async (req, res) => {
  ...
  const runId = String(req.body?.run_id || '').trim();
  if (!manifestIdInput && !runId)
    return res.status(400).json({error: 'manifest_id or run_id required'});
...
app.post('/api/messenger/check-in', async (req, res) => {
  ...
  if (!runId || !checkpointId)
    return res.status(400).json({error: 'run_id and checkpoint_id required'});
  ...
  const {data: existingCheckins} = await supabase
    ...
    .eq('run_id', runId);
  if (
    (existingCheckins || []).some(item => item.checkpoint_id === checkpointId)
  ) {
    return res.json({
      ok: true,
...
app.post('/api/messenger/finish', async (req, res) => {
  ...
  const missing = (manifest?.manifest?.checkpoints || []).filter(
...
app.all('/api/messenger/run-state', async (req, res) => {
  ...
  const runId =
    (req.method === 'GET' ? req.query.run_id : req.body?.run_id) || '';
  if (!runId) return res.status(400).json({error: 'run_id required'});
  ...
  const {data: proofs, error: proofsError} = await supabase
    .from('messenger_proof_posts')
    .select(
      'id, checkpoint_id, checkpoint_name, public_url, location_label, is_public, created_at',
    )
    .eq('run_id', runId)
    .order('created_at', {ascending: true});
...
app.post('/api/messenger/proof', async (req, res) => {
  ...
  if (!(checkins || []).some(row => row.checkpoint_id === checkpointId)) {
    return res
      .status(400)
      .json({error: 'check in at the checkpoint before posting proof'});
  }
  ...
  if (existingProofsError)
    return res.status(500).json({error: existingProofsError.message});
...
app.get('/api/wall', async (req, res) => {
  ...
  const checkpointCount =
    Number(req.query.checkpoints || req.query.checkpoint_count || 0) || null;
  try {
    const {data: posts, error} = await supabase
      .from('messenger_proof_posts')
      .select(
        'id, run_id, manifest_id, user_id, rider_name, city_name, city_slug, checkpoint_name, location_label, is_public, created_at, public_url, storage_path, bike_name, bike_ratio',
      )
      .eq('is_public', true)
    ...
    const picked = [...groups.values()]
      ...
      .slice(0, 40);
    const manifestIds = [
      ...new Set(picked.map(post => post.manifest_id).filter(Boolean)),
    ];
    const {data: manifests, error: manifestsError} = manifestIds.length
      ? await supabase
          .from('messenger_manifests')
          .select('id, manifest_title, checkpoint_count')
          .in('id', manifestIds)
      : {data: [], error: null};
    if (manifestsError)
      return res.status(500).json({error: manifestsError.message});
    const manifestMap = new Map(
      (manifests || []).map(manifest => [manifest.id, manifest]),
    );
    const enriched = picked
      ...
      .filter(
        post =>
          !checkpointCount ||
          Number(post.checkpoint_count || 0) === checkpointCount,
      )
...
const handleNightRideFeed = async (req, res, input = {}) => {
  const city = String(
    input.city ?? req.body?.city ?? req.query?.city ?? '',
  ).trim();
  try {
    const query = supabase
      .from('night_ride_posts')
      .select(
        'id, user_id, rider_name, crew_name, city_name, route_title, distance_km, caption, image_url, aspect_ratio, moderation_status, created_at',
      )
      .eq('is_public', true)
...
app.get('/api/rider-profile', async (req, res) => {
  ...
  const [
    ...
    supabase
      .from('messenger_proof_posts')
      .select(
        'id, user_id, rider_name, city_name, city_slug, checkpoint_name, location_label, public_url, created_at, bike_name, bike_ratio',
      )
      .eq('user_id', userId)
      .eq('is_public', true)
      .is('archived_at', null)
  ...
  if (proofsRes.error)
    return res.status(500).json({error: proofsRes.error.message});
  if (runsRes.error)
    return res.status(500).json({error: runsRes.error.message});

  const proofs = proofsRes.data || [];
  const runs = runsRes.data || [];
  const riderName =
    profileRes.data?.rider_name || proofs[0]?.rider_name || 'Rider';
  const quarterBoard = buildQuarterLeaderboard({
    proofs: quarterProofsRes.data || [],
    finishedRuns: quarterRunsRes.data || [],
  });
  const quarterEntry =
    quarterBoard.find(entry => entry.user_id === userId) || null;
  ...
  const topCity =
    [...proofsByCity.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] || '';
  const bestFinish =
    runs
  ...
  return res.json({
    profile: {
      ...
      rider_name: riderName,
      home_location:
        profileRes.data?.home_location || proofs[0]?.city_name || '',
    ...
    stats: {
      ...
      cities: new Set(proofs.map(proof => proof.city_name).filter(Boolean))
        .size,
      top_city: topCity,
...
app.post('/api/admin/proofs', requireAdmin, async (_req, res) => {
  const {data, error} = await supabase
    .from('messenger_proof_posts')
    .select(
      'id, rider_name, city_name, checkpoint_name, is_public, created_at, public_url, storage_path, archived_at',
    )
    .order('created_at', {ascending: false})
    .limit(250);
  if (error) return res.status(500).json({error: error.message});
...
async function handlePublicLeaderboard(req, res, input = {}) {
  const city = String(input.city ?? req.query.city ?? '')
    .trim()
    .toLowerCase();
  const country = String(input.country ?? req.query.country ?? '')
    .trim()
    .toLowerCase();
  const checkpointCount =
    Number(
      input.checkpoint_count ??
        req.body?.checkpoint_count ??
        req.query.checkpoints ??
        req.query.checkpoint_count ??
        0,
    ) || null;
  ...
  if (!city && country && cityScope.length)
    proofsQuery.in('city_slug', cityScope);
  let manifestIds = [];
  if (city || (country && cityScope.length) || checkpointCount) {
    ...
    else if (country && cityScope.length)
      manifestsQuery.in('city_slug', cityScope);
    if (checkpointCount) manifestsQuery.eq('checkpoint_count', checkpointCount);
    const manifestsRes = await manifestsQuery;
    if (manifestsRes.error)
      return res.status(500).json({error: manifestsRes.error.message});
    manifestIds = (manifestsRes.data || [])
      .map(item => item.id)
      .filter(Boolean);
  ...
  const leaders = buildQuarterLeaderboard({
    proofs: proofsRes.data || [],
    finishedRuns: runsRes.data || [],
  }).slice(0, 25);
...
app.get('/api/leaderboard', async (req, res) =>
  handlePublicLeaderboard(req, res),
);
...
async function handleGetCityLanes(_req, res) {
  ...
  const [packsRes, checkpointsRes, requestsRes, proofsRes] = await Promise.all([
    supabase
      .from('city_packs')
      .select(
        'id,slug,name,route_note,finish_label,safety_note,is_active,created_at',
      )
  ...
  if (requestsRes.error)
    return res.status(500).json({error: requestsRes.error.message});
  if (proofsRes.error)
    return res.status(500).json({error: proofsRes.error.message});
...
app.post('/api/messenger/share', async (req, res) => {
  ...
  if (code) {
    ...
    if (sourceManifestError)
      return res.status(500).json({error: sourceManifestError.message});
    if (!sourceManifest)
      return res.status(404).json({error: 'source manifest missing'});
  ...
  if (challengeInsertError)
    return res.status(500).json({error: challengeInsertError.message});
  await supabase
    .from('messenger_manifests')
    .update({source_challenge_id: challenge.id})
    .eq('id', manifestId);
...
app.all('/api/messenger/leaderboard', async (req, res) => {
  ...
  let challengeId =
    (req.method === 'GET' ? req.query.challenge_id : req.body?.challenge_id) ||
    '';
  const manifestId =
    (req.method === 'GET' ? req.query.manifest_id : req.body?.manifest_id) ||
    '';
