...
export async function onRequest({request, env}) {
  ...
  try {
    ...
    const crewName = String(body.crew_name || '')
      .trim()
      .slice(0, 80);
    const rideCity = String(body.ride_city || '')
      .trim()
      .slice(0, 80);
    ...
    if (mode === 'loop') {
      ...
      routeUrl = buildNightRideMapsUrl({
        origin: start,
        destination: start,
      ...
      rows = await supabaseRequest(env, 'night_ride_sessions', {
        ...
        body: JSON.stringify({
          ...
          creator_rider_name:
            ...
            riderLabelFromEmail(user.email || ''),
    ...
    if (session?.id) {
      await supabaseRequest(env, 'night_ride_participants', {
        ...
        body: JSON.stringify({
          ...
          joined_via: 'creator',
          credits_spent:
            sessionType === 'crew' ? NIGHT_RIDE_CREW_BUILD_COST : 1,
        }),
      }).catch(() => null);
    }
    ...
    return json(
      {
        error:
          err instanceof Error ? err.message : 'Night Ride generation failed',
      },
      {status: 500},
    );
  }
