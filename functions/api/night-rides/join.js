import { json, parseJSON, getAuthUser, isAdminEmail, supabaseRequest } from "../../_utils.js";
...
export async function onRequest({ request, env }) {
  ...
  if (!session) return json({ error: "night ride code not found" }, { status: 404 });
  if (session.status !== "open") return json({ error: "night ride is closed" }, { status: 409 });
  if (session.session_type !== "crew") return json({ error: "single night rides do not take join codes" }, { status: 409 });
