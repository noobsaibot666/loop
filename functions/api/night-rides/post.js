...
export async function onRequest({request, env}) {
  ...
  const aspectRatio =
    String(body.aspect_ratio || '1:1').trim() === '16:9' ? '16:9' : '1:1';
  ...
  if (!sessionId || !imageUrl || !storagePath) {
    return json(
      {error: 'session_id, image_url, and storage_path required'},
      {status: 400},
    );
  }
  ...
  if (!session)
    return json({error: 'night ride session not found'}, {status: 404});
