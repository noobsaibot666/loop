...
export async function onRequest({request, env}) {
  try {
    ...
    const ghostEnabled =
      body?.ghost_enabled !== false &&
      body?.ghost_enabled !== 'false' &&
      body?.ghost_enabled !== 0;
    if (
    ...
    const startPoint =
      Number.isFinite(Number(start_lat)) && Number.isFinite(Number(start_lng))
        ...
        : null;
    const dbPack = await getPublishedCityPackByCity(
      env,
      normalizeCitySlug(city),
    );
    const dbCheckpoints = dbPack
      ? await getPackCheckpoints(env, dbPack.id, true)
      : [];
    const dbBuilt =
