...
export async function onRequest({ request, env }) {
  ...
  const manifestIds = [...new Set((entries || []).map((e) => e.manifest_id).filter(Boolean))];
  ...
  for (const proof of proofs || []) {
    if (!proofsByManifest[proof.manifest_id]) proofsByManifest[proof.manifest_id] = [];
  ...
  for (const entry of entries || []) {
    if (!entriesByChallenge[entry.challenge_id]) entriesByChallenge[entry.challenge_id] = [];
