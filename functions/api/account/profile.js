...
export async function onRequest({ request, env }) {
  ...
  const payload = {
    ...
    age: body?.age ? Math.min(120, Math.max(1, Number(body.age) || 0)) || undefined : undefined,
  ...
  try {
    ...
    if (normalized.includes("user_profiles") || normalized.includes("primary_bike_id")) {
    ...
    if (normalized.includes("user_bikes")) {
      return json({ error: "Bike fields are not ready in Supabase yet. Apply user_bikes.sql first." }, { status: 500 });
    }
