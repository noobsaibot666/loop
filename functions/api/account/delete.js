...
export async function onRequestDelete({ request, env }) {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      await supabaseRequest(env, `${table}?user_id=eq.${encodeURIComponent(user_id)}`, {
        method: "DELETE"
      }).catch(err => {
    ...
    await supabaseAdminAuthRequest(env, `admin/users/${encodeURIComponent(user_id)}`, {
      method: "DELETE"
    });

    return json({ ok: true, message: "Account and data purged successfully." });
  } catch (error) {
    return json({ error: error instanceof Error ? error.message : "Account deletion failed" }, { status: 500 });
  }
}
