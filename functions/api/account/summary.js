...
export async function onRequest({request, env}) {
  ...
  try {
    ...
    const challengeEntriesByChallenge = challengeIds.length
      ...
      : [];
    const challengeManifestIds = [
      ...new Set(challengeEntriesByChallenge.map(entry => entry.manifest_id)),
    ];
    ...
    const purchases = [
      ...
      .slice(0, 5);
    const challengeManifestSet = new Set(
      userManifests
    ...
    const badges = deriveBadges({
      quarterStats: {
        rank:
          quarterLeaderboard.find(entry => entry.user_id === user.id)?.rank ||
          null,
    ...
    const userQuarterRank =
      quarterLeaderboard.find(entry => entry.user_id === user.id)?.rank || null;
    const bikes = (bikeRows || []).map((bike, index) => ({
      ...
      is_default: Boolean(
        bike.is_default ||
        (profileRows?.[0]?.primary_bike_id &&
          bike.id === profileRows[0].primary_bike_id) ||
    ...
    return json({
      ...
      profile: profileRows?.[0]
        ? {
            ...profileRows[0],
            primary_bike_id:
              profileRows[0].primary_bike_id || defaultBike?.id || null,
            bike_name: defaultBike?.bike_name || profileRows[0].bike_name || '',
            bike_ratio:
              defaultBike?.bike_ratio || profileRows[0].bike_ratio || '',
          }
        : {
            user_id: user.id,
            rider_name: '',
      ...
      community_membership: sanitizeMembershipForClient(
        communityMembershipRows?.[0] || null,
      ),
      alleycat: {
      ...
      alleycat_history: buildAlleycatHistory({
        manifests: challengeManifests.length
          ? [
              ...userManifests,
