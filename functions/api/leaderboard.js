...
export async function onRequest({request, env}) {
  ...
  const cityScope = country
    ? Object.entries(CITY_COUNTRY_MAP)
        ...
        .map(([slug]) => slug)
    : [];
  const needsManifestFilter = Boolean(
    city || (country && cityScope.length) || checkpointCount,
  );
  let manifestIds = [];
  if (needsManifestFilter) {
    ...
    if (city) manifestFilters.push(`city_slug=eq.${encodeURIComponent(city)}`);
      ...
      manifestFilters.push(
        `city_slug=in.(${cityScope.map(slug => encodeURIComponent(slug)).join(',')})`,
      );
    if (checkpointCount)
      manifestFilters.push(`checkpoint_count=eq.${checkpointCount}`);
  ...
  const avatarMap = new Map(
    (avatarProfiles || []).map(p => [p.user_id, p.avatar_url || null]),
  );
