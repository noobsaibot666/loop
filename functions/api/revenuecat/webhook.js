...
const acceptedGrantEventTypes = new Set([
  'INITIAL_PURCHASE',
...
const parseAmountCents = (event, productRow) => {
  ...
  if (!Number.isFinite(amount) || amount <= 0)
    return Number(productRow?.price_cents || 0);
  return Math.round(amount * 100);
};

const parsePurchasedAt = event => {
  const value = Number(
    event?.purchased_at_ms || event?.event_timestamp_ms || 0,
  );
  if (Number.isFinite(value) && value > 0) return new Date(value).toISOString();
  const fallback = String(
    event?.purchased_at || event?.event_timestamp || '',
  ).trim();
