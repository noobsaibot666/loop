...
export async function onRequest({request, env}) {
  ...
  const [profileRows, publicProofs, finishedRuns, quarterProofs, quarterRuns] =
    await Promise.all([
      supabaseRequest(
        env,
  ...
  const topCityEntry =
    [...proofsByCity.values()].sort(
      (a, b) => b.proof_count - a.proof_count,
    )[0] || null;
  ...
  const uniqueProofDays = [
    ...new Set(proofs.map(proof => toDayStamp(proof.created_at))),
  ].sort((a, b) => b - a);
  ...
  const runManifestIds = [
    ...new Set((runs || []).map(run => run.manifest_id).filter(Boolean)),
  ];
  let runManifests = [];
  ...
  const challengeIds = [
    ...new Set(
      (challengeEntries || []).map(entry => entry.challenge_id).filter(Boolean),
    ),
  ];
  const manifestIds = [
    ...new Set(
      (challengeEntries || []).map(entry => entry.manifest_id).filter(Boolean),
    ),
  ];
  ...
  const lastProofAt = proofs[0]?.created_at
    ? new Date(proofs[0].created_at).getTime()
    : 0;
  const lastRunAt = runs[0]?.finished_at
    ? new Date(runs[0].finished_at).getTime()
    : 0;
  const lastActiveAt = Math.max(lastProofAt, lastRunAt) || null;
  const recentRuns = runs.slice(0, 6).map(run => {
    const manifest = (runManifests || []).find(
      item => item.id === run.manifest_id,
    );
    const ghostSeconds =
    ...
    return {
      ...
      ghost_delta:
        ghostSeconds !== null && typeof run.finish_seconds === 'number'
          ? run.finish_seconds - ghostSeconds
          : null,
    };
  });
  const cityClusters = cityBreakdown.slice(0, 3).map(city => ({
    ...
    posts: proofs
      .filter(
        proof =>
          (proof.city_slug || proof.city_name) ===
          (city.city_slug || city.city_name),
      )
      .slice(0, 3),
  }));
  ...
  if (topCitySlug) {
    const cityQuarterProofs = (quarterProofs || []).filter(
      proof => (proof.city_slug || '').trim().toLowerCase() === topCitySlug,
    );
    let cityQuarterRuns = [];
    ...
    const cityManifestIds = (cityQuarterManifests || [])
      .map(item => item.id)
      .filter(Boolean);
    ...
    const cityBoard = buildQuarterLeaderboard({
      proofs: cityQuarterProofs,
      finishedRuns: cityQuarterRuns || [],
    });
