...
function getPackReadiness(pack, countData, districtsByPack) {
  ...
  if (copyReady && activeCheckpointCount >= 4 && districtCount >= 3)
    return 'ready';
...
export async function onRequest({request, env}) {
  ...
  const [
    ...
    supabaseRequest(env, 'user_credits?select=user_id,credits,free_used', {
      method: 'GET',
    }),
    supabaseRequest(
      env,
      'stripe_sessions?select=session_id,status,amount_cents,created_at&order=created_at.desc&limit=20',
      {method: 'GET'},
    ),
    supabaseRequest(
      env,
      'messenger_manifests?select=id,city_name,manifest_title,checkpoint_count,ghost_seconds,created_at',
      {method: 'GET'},
    ),
    supabaseRequest(
      env,
      'messenger_runs?select=id,user_id,manifest_id,status,finish_seconds,finished_at,bike_name,bike_ratio',
      {method: 'GET'},
    ),
    ...
    supabaseRequest(
      env,
      'city_packs?select=id,name,is_active,route_note,finish_label,safety_note',
      {method: 'GET'},
    ).catch(() => []),
    supabaseRequest(env, 'city_checkpoints?select=pack_id,is_active,district', {
      method: 'GET',
    }).catch(() => []),
    supabaseRequest(
      env,
  ...
  for (const manifest of manifests || []) {
    ...
    const current = cityPulseMap.get(city) || {
      city_name: city,
      manifests: 0,
      proofs: 0,
      public_proofs: 0,
  ...
  const cityPulse = Array.from(cityPulseMap.values())
    .sort(
      (a, b) =>
        b.public_proofs - a.public_proofs ||
        b.manifests - a.manifests ||
  ...
  for (const proof of proofs) {
    ...
    if (!current.last_at || String(proof.created_at) > current.last_at)
      current.last_at = proof.created_at || current.last_at;
    riderWatchMap.set(riderId, current);
  }
  const abuseWatch = Array.from(riderWatchMap.values())
    .filter(
      row =>
        row.proof_count >= 3 || row.hidden_count > 0 || row.archived_count > 0,
    )
    .sort(
      (a, b) =>
        b.hidden_count - a.hidden_count ||
        b.archived_count - a.archived_count ||
  ...
  const checkoutFailures = recentSessions.filter(
    session =>
      !['checkout_created', 'completed', 'paid'].includes(
        String(session.status || '').toLowerCase(),
      ),
  ).length;
  const completionRate = runs?.length
    ? Math.round((finishedRuns / runs.length) * 100)
    : 0;
  ...
  const snapshots = [
    {
      ...
      manifests: (manifests || []).filter(
        item => item.created_at && new Date(item.created_at) >= last7,
      ).length,
      proofs: proofs.filter(
        item => item.created_at && new Date(item.created_at) >= last7,
      ).length,
      topups: recentSessions.filter(
        item => item.created_at && new Date(item.created_at) >= last7,
      ).length,
    },
    {
      label: 'Last 30 days',
      manifests: (manifests || []).filter(
        item => item.created_at && new Date(item.created_at) >= last30,
      ).length,
      proofs: proofs.filter(
        item => item.created_at && new Date(item.created_at) >= last30,
      ).length,
  ...
  return json({
    ...
    metrics: {
      ...
      live_city_packs: packReadiness.filter(pack => pack.readiness === 'live')
        .length,
      ready_city_packs: packReadiness.filter(pack => pack.readiness === 'ready')
        .length,
    },
    ...
    failure_signals: [
      ...
      {
        label: 'Ready city packs',
        value: packReadiness.filter(pack => pack.readiness === 'ready').length,
      },
    ],
    quarter: {
      label: quarter.label,
