...
export async function onRequest({ request, env }) {
  ...
  const [purchaseEvents, productCatalog, stripeSessions, userCredits] = await Promise.all([
    supabaseRequest(
      env,
  ...
  return json({
    ...
    metrics: {
      ...
      product_catalog_seeded: Array.isArray(productCatalog) && productCatalog.length > 0,
    },
  });
}
