...
const sendEmailForEventType = async ({
  env,
  request,
  eventType,
  membership,
  ...
  if (eventType === 'email_activation_failed') {
    return sendCommunityActivatedEmail({
      env,
      request,
      membership,
      user: recipient,
  ...
  if (eventType === 'email_discord_linked_failed') {
    return sendCommunityDiscordLinkedEmail({
      env,
      request,
      membership,
      user: recipient,
  ...
  if (eventType === 'email_cancellation_failed') {
    return sendCommunityCanceledEmail({
      env,
      request,
      membership,
      user: recipient,
...
export async function onRequest({request, env}) {
  ...
  if (!event.user_id)
    return json({error: 'event has no user_id'}, {status: 400});
  ...
  try {
    ...
    if (
      String(event.event_type || '').startsWith('email_') &&
      String(event.event_type || '').endsWith('_failed')
    ) {
