...
export async function onRequest({request, env}) {
  ...
  if (String(body.action || '') === 'ai_draft') {
    ...
    const requests = await supabaseRequest(
      env,
      `city_requests?id=eq.${encodeURIComponent(requestId)}&select=*`,
      {
        method: 'GET',
      },
    );
    ...
    if (!cityName || !slug)
      return json(
        {error: 'request needs a city name before drafting'},
        {status: 400},
      );
    ...
    const packs = await supabaseRequest(
      env,
      `city_packs?slug=eq.${encodeURIComponent(slug)}&select=*`,
      {method: 'GET'},
    ).catch(() => []);
    ...
    const updatedRows = await supabaseRequest(
      env,
      `city_requests?id=eq.${encodeURIComponent(requestId)}`,
      {
        method: 'PATCH',
