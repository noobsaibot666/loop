...
export async function onRequest({ request, env }) {
  ...
  if (!customerId && membership.stripe_subscription_id) {
    try {
      ...
      return json(
        {
          error: error instanceof Error ? error.message : "Could not resolve subscription customer",
        },
        { status: 500 }
      );
    }
  ...
  if (!customerId) return json({ error: "customer portal unavailable" }, { status: 400 });
