...
const updateMembershipBySubscription = async (
  ...
  const membership = buildMembershipUpsert({
    ...
    checkoutSession: {
      ...
      metadata: {
        ...
        discord_invite_url:
          subscription?.metadata?.discord_invite_url || COMMUNITY_INVITE_URL,
      },
    },
    subscription,
  });
  let nextMembership = {
    ...
    price_cents:
      overrides.price_cents ?? membership.price_cents ?? COMMUNITY_PRICE_CENTS,
  };
      ...
      await recordCommunityEvent(env, {
        ...
        details: {
          ...
          stripe_subscription_id:
            mergedMembership.stripe_subscription_id || null,
        },
      }).catch(() => null);
    ...
    if (user_id && creditAdd > 0 && sessionId) {
      ...
      try {
        ...
        if (
          ...
          existingSession[0]?.status === 'credited'
        ) {
          return json({received: true, duplicate: true});
        }
      } catch {}
