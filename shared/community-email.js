...
const formatPrice = (amount = COMMUNITY_PRICE_CENTS, currency = COMMUNITY_CURRENCY) =>
...
const buildShell = ({ eyebrow, title, body, details, ctaLabel, ctaUrl, footnote }) => `
...
const getEmailConfig = (env, request) => {
  ...
  return {
    ...
    guildUrl: env?.DISCORD_GUILD_ID ? buildDiscordGuildUrl(env.DISCORD_GUILD_ID) : null,
  };
};
...
const buildPlanDetails = (membership) => [
  {
    label: "Pass",
    value: membership?.plan_code === COMMUNITY_PLAN_CODE ? "Community pass" : membership?.plan_code || "Community pass",
  },
  {
    label: "Price",
...
export const sendCommunityActivatedEmail = async ({ env, request, membership, user }) => {
  ...
  await postEmail(config, {
    ...
    html: buildShell({
      ...
      footnote: "Next step: connect Discord from your account if you have not linked it yet.",
    }),
    text: textShell({
      title: "Community pass active",
...
export const sendCommunityDiscordLinkedEmail = async ({ env, request, membership, user }) => {
  const config = getEmailConfig(env, request);
  const recipient = normalizeRecipient(user);
  if (!config || !recipient.email) return false;
  const details = [
  ...
  await postEmail(config, {
    ...
    html: buildShell({
      ...
      footnote: "Use the account page any time to manage billing or reconnect Discord.",
    }),
    text: textShell({
      title: "Discord linked",
...
export const sendCommunityCanceledEmail = async ({ env, request, membership, user }) => {
  const config = getEmailConfig(env, request);
  const recipient = normalizeRecipient(user);
  if (!config || !recipient.email) return false;
  const details = [
    ...
    { label: "Ends", value: formatDate(membership?.current_period_end) || "Access removed" },
  ];
  await postEmail(config, {
    to: recipient.email,
    ...
    html: buildShell({
      ...
      ctaUrl: `${config.appUrl}/account`,
      footnote: "If this was not expected, open your account and check the billing details.",
    }),
    text: textShell({
      title: "Pass canceled",
