const getRiderName = (userId, proofs = []) => {
  const sample = (proofs || []).find(
    proof => proof.user_id === userId && proof.rider_name,
  );
...
const buildAlleycatHistory = ({manifests = [], runs = [], proofs = []}) =>
  [...manifests]
    ...
    .map(manifest => {
      ...
      const finishedRuns = manifestRuns.filter(
        run =>
      ...
      const bestRun =
        [...finishedRuns].sort(
          (left, right) =>
            (left.finish_seconds || 0) - (right.finish_seconds || 0),
        )[0] || null;
      ...
      return {
        ...
        ghost_delta:
          bestRun && ghostSeconds !== null
            ? bestRun.finish_seconds - ghostSeconds
            : null,
    ...
    .map(entry => {
      ...
      const bestRun =
        runs
          .filter(
            run =>
              run.manifest_id === entry.manifest_id &&
              run.status === 'finished' &&
      ...
      const rivalCount = allEntries.filter(
        item =>
          item.challenge_id === entry.challenge_id && item.user_id !== userId,
      ).length;
      return {
...
const buildSharedRiders = ({
  ...
  for (const entry of allEntries) {
    if (entry.user_id === userId || !userChallengeIds.has(entry.challenge_id))
      continue;
    ...
    current.last_joined_at =
      new Date(entry.joined_at) > new Date(current.last_joined_at)
        ? entry.joined_at
        : current.last_joined_at;
    const manifest = manifests.find(item => item.id === entry.manifest_id);
  ...
  return [...map.values()]
    ...
    .sort((left, right) => {
      if (right.shared_challenges !== left.shared_challenges)
        return right.shared_challenges - left.shared_challenges;
      return (
        new Date(right.last_joined_at).getTime() -
        new Date(left.last_joined_at).getTime()
      );
    })
    .slice(0, 8);
};
