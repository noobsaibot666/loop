...
const getChallengeExpiry = createdAt => {
  ...
  return new Date(
    start + CHALLENGE_EXPIRY_DAYS * 24 * 60 * 60 * 1000,
  ).toISOString();
};
...
const deriveChallengeStatus = (challenge, leaderboard = []) => {
  ...
  const hasOpenEntrants = leaderboard.some(
...
const buildChallengeSummary = ({challenge, leaderboard = [], userId}) => {
  ...
  const rivals = leaderboard.filter(
    entry => entry.user_id !== userId && entry.best_seconds !== null,
  );
  const nearestRival =
    you && you.best_seconds !== null && rivals.length
      ? [...rivals].sort(
          (left, right) =>
            Math.abs(left.best_seconds - you.best_seconds) -
            Math.abs(right.best_seconds - you.best_seconds),
        )[0]
      : rivals[0] || null;
  ...
  return {
    ...
    expires_at: challenge?.created_at
      ? getChallengeExpiry(challenge.created_at)
      : null,
