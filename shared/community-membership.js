...
export const isMembershipActive = (membership, now = Date.now()) => {
  ...
  if (!ACTIVE_STATUSES.has(String(membership.status || "").toLowerCase())) return false;
...
export const deriveMembershipAccessState = (membership, now = Date.now()) => {
  ...
  if (!status || status === "pending" || status === "checkout_created") return "pending";
  if (status === "canceled" || status === "incomplete_expired" || status === "unpaid") return "inactive";
  return "restricted";
};
...
export const buildMembershipUpsert = ({ userId, checkoutSession, subscription, inviteUrl = COMMUNITY_INVITE_URL }) => ({
  ...
  stripe_subscription_id: subscription?.id || checkoutSession?.subscription || null,
  stripe_checkout_session_id: checkoutSession?.id || null,
  plan_code: checkoutSession?.metadata?.plan_code || subscription?.metadata?.plan_code || COMMUNITY_PLAN_CODE,
