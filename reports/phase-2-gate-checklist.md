...
- [x] Month archive works correctly in admin and archived posts disappear from the public wall
...
- [x] Quarter board renders correctly on real rider data
- [x] Ghost targets feel challenging but still fair on real rider runs

## Admin tooling
- [x] City request flow needs full retest
  Rider side: user can request a city or riding area from the app
  Admin side: requests show up clearly and can be reviewed
- [x] City pack create/edit flow needs final UX cleanup and retest
- [x] Preview manifest flow in admin needs retest with current city packs
- [x] AI draft output review gate needs explicit verification
  Confirm drafts never auto-publish without admin action

## QA-only settings to remove before wider release
- [x] Remove temporary Alleycat QA mode
