...
import {
  ...
  MessengerRun,
  AlleycatChallenge,
  AlleycatChallengeSummary,
  AlleycatLeaderboardEntry,
  ProofDraft,
  MessengerCheckpoint,
  MessengerProof,
  CheckpointChallenge,
...
const validateChallengeAnswer = (
  challenge: CheckpointChallenge | undefined,
  input: string,
): boolean => {
  ...
  return ((challenge as any).alt_answers || []).some(
    (alt: string) => norm(alt) === given,
  );
};
import {
  ALLEYCAT_STORAGE_KEY,
  ALLEYCAT_PROOF_DRAFTS_KEY,
  API_BASE,
  PROOF_BUCKET,
} from '../config';
    ...
    (set, get) => ({
      ...
      generateManifest: async userId => {
        ...
        try {
          ...
          if (!origin) {
            const geo = await postJSON<any>('/api/geocode', {
          ...
          const rangeKm =
            config.unit === 'km' ? config.range : config.range * 1.60934;
          const storedLocale =
        ...
        try {
          ...
          set({
            run: {
              ...
              checkins: (data.checkins || []).reduce(
                (acc: Record<string, string>, row: any) => {
                  if (row?.checkpoint_id && row?.checked_in_at)
                    acc[row.checkpoint_id] = row.checked_in_at;
                  return acc;
                },
                {},
              ),
      ...
      finishRun: async () => {
        ...
        if (!run) return;
        try {
          const data = await postJSON<any>('/api/messenger/finish', {
            run_id: run.runId,
          });
          set({
            run: {
      ...
      abandonRun: async () => {
        ...
        try {
          ...
          set({
            ...
            status: 'alleycat.status.abandoned',
          });
        } catch (e) {
      ...
      uploadProof: async (checkpoint, file) => {
        ...
        try {
          ...
          if (!draft && file) {
            ...
            const {
              data: {publicUrl},
            } = supabase.storage.from(PROOF_BUCKET).getPublicUrl(path);
            draft = {
              runId: run.runId,
              checkpointId,
              storagePath: path,
            ...
            set(s => ({
              proofDrafts: {...s.proofDrafts, [checkpointId]: draft},
            }));
          }
      ...
      submitAnswer: async (checkpoint, answer) => {
        const {run} = get();
        if (!run) return 'wrong';
        if (!validateChallengeAnswer(checkpoint.challenge, answer))
          return 'wrong';
      ...
      createShareCode: async () => {
        ...
        try {
          ...
          const nextCode = String(data.code || data.source_code || '')
            .trim()
            .toUpperCase();
          set({
      ...
      fetchRunState: async userId => {
        ...
        if (!run) return;
        try {
          const data = await postJSON<any>('/api/messenger/run-state', {
            run_id: run.runId,
          });
          set({
            manifest: data.manifest,
            ...
            run: {
              ...
              checkins: (data.run.checkins || []).reduce(
                (acc: Record<string, string>, row: any) => {
                  if (row?.checkpoint_id && row?.checked_in_at)
                    acc[row.checkpoint_id] = row.checked_in_at;
                  return acc;
