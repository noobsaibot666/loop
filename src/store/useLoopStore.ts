...
const sampleLoopMapsWaypoints = (
  ...
  for (let index = 0; index < waypointCount; index += 1) {
    ...
    const sample = pointAtDistance(
      points,
      cumulative,
      total * (0.08 + ratio * 0.82),
    );
  ...
  candidates
    ...
    .forEach(({point}) => {
      ...
      if (picked.some(existing => haversineKm(existing, point) < minSpacingKm))
        return;
  ...
  if (picked.length < 4) {
    [0.14, 0.3, 0.48, 0.64, 0.8, 0.9].forEach(ratio => {
      ...
      if (
        picked.some(
          existing => haversineKm(existing, sample) < minSpacingKm * 0.8,
        )
      )
        return;
      if (keys.has(key)) return;
  ...
  return picked
    .sort((left, right) => {
      ...
      const leftIndex = points.findIndex(
        point => `${point.lat.toFixed(4)},${point.lng.toFixed(4)}` === leftKey,
      );
      const rightIndex = points.findIndex(
        point => `${point.lat.toFixed(4)},${point.lng.toFixed(4)}` === rightKey,
      );
...
const buildGoogleMapsLoopUrl = (
  ...
  const points = routeWaypoints.filter(
...
const buildFallbackLoopWaypoints = (
  ...
  return bearings.map((bearing, index) => {
    ...
    const distance = Math.max(
      0.9,
      Math.min(radiusBase * radiusFactors[index], radiusBase * 1.25),
    );
    ...
    const lat2 = Math.asin(
      Math.sin(lat1) * Math.cos(distance / earthRadiusKm) +
        Math.cos(lat1) *
          Math.sin(distance / earthRadiusKm) *
          Math.cos(bearingRad),
    );
    const lng2 =
      lng1 +
    ...
    (set, get) => ({
      ...
      fetchSuggestions: async (text: string) => {
        ...
        try {
          ...
          const results =
            geo?.features?.slice(0, 5).map((f: any) => ({
      ...
      generateLoop: async (
        ...
        try {
          ...
          const serverWaypoints = Array.isArray(loop?.sampled_waypoints)
            ...
            : [];
          const waypoints = hasUsableLoopWaypoints(serverWaypoints)
            ? serverWaypoints
            : sampleLoopMapsWaypoints(coords, distanceKm);
          ...
          const resolvedWaypoints = hasUsableLoopWaypoints(waypoints)
            ...
            : buildFallbackLoopWaypoints(
                ...
                distanceKm,
                Number(
                  loop?.candidate_seed || Math.floor(Math.random() * 1000),
                ),
              );

          if (
            (!routeUrl || !routeUrl.includes('/maps/dir/')) &&
            ...
            const fallbackWaypoints = bearings
              .map(bearing => {
                ...
                const lat2 = Math.asin(
                  Math.sin(lat1) *
                    Math.cos(fallbackDistanceKm / earthRadiusKm) +
                    Math.cos(lat1) *
                      Math.sin(fallbackDistanceKm / earthRadiusKm) *
                      Math.cos(bearingRad),
                );
                const lng2 =
                  lng1 +
