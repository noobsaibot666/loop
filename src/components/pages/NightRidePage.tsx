...
import {Users, Zap, Camera, Compass, Image as ImageIcon, X} from 'lucide-react';
...
const NightRidePage = ({
  ...
  const searchLocations = async (
    query: string,
    setResults: (results: Suggestion[]) => void,
  ) => {
    ...
    try {
      ...
      }>('/api/geocode', {text: query});
      const suggestions =
        data.features?.slice(0, 5).map(feature => ({
          label:
            feature.properties?.label || feature.properties?.name || 'Unknown',
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => searchLocations(startLabel, setStartSuggestions),
      400,
    );
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => searchLocations(endLabel, setEndSuggestions),
      400,
    );
    return () => clearTimeout(timer);
  }, [endLabel]);
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: MouseEvent) => {
      ...
      if (
        startSuggestionRef.current &&
        !startSuggestionRef.current.contains(target)
      ) {
        setStartSuggestions([]);
      }
      if (
        endSuggestionRef.current &&
        !endSuggestionRef.current.contains(target)
      ) {
        setEndSuggestions([]);
      }
    };
  ...
  const handleBuild = async () => {
    ...
    setStatus('');
    try {
      const members = crewMembersInput
        .split(',')
        .map(m => m.trim())
        .filter(Boolean);
      const payload = {
      ...
      const results = await postJSON<{session: NightRideSession}>(
        '/api/night-rides/create',
        payload,
      );
      if (results?.session) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setStatus(
        error instanceof Error ? error.message : t('night.messages.joinFailed'),
      );
    } finally {
      setIsJoining(false);
    }
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      ...
      const response = await postJSON<{post: NightRidePost}>(
        '/api/night-rides/post',
        {
      ...
      setPostStatus(
        error instanceof Error ? error.message : t('night.messages.postFailed'),
      );
    } finally {
      setIsPosting(false);
    }
  ...
  const flow = useMemo(
    () => [
      {
        number: '01',
        title: t('night.flow1.title'),
        body: t('night.flow1.body'),
        icon: <Users size={22} />,
      },
      {
        number: '02',
        title: t('night.flow2.title'),
        body: t('night.flow2.body'),
        icon: <Compass size={22} />,
      },
      {
        number: '03',
        title: t('night.flow3.title'),
        body: t('night.flow3.body'),
        icon: <Zap size={22} />,
      },
      {
        number: '04',
        title: t('night.flow4.title'),
        body: t('night.flow4.body'),
          ...
          <div className="glass-card form-card night-ride-shell">
            ...
            <div className="form-body">
                ...
                <div className="field-grid-two">
                  ...
                  <label className="field">
                    ...
                    <input
                      ...
                      onChange={event =>
                        setCrewMembersInput(event.target.value)
                      }
              ...
              <div className="form-section section-block">
                <label className="field">
                  ...
                  {riderBikes.length > 1 ? (
                    ...
                    <div className="account-note">
                      {t('common.standardBike')}: {riderBikes[0].bike_name} ·{' '}
                ...
                <label className="field range-field">
                  ...
                  <input
                    ...
                    style={
                      {
                        '--range-progress': `${((distance - (unit === 'km' ? 5 : 3)) / (unit === 'km' ? 35 : 22)) * 100}%`,
                      } as any
                    }
                  />
                  <div className="range-labels">
                    ...
                    <div className="range-focus-card">
                      <strong>
                        {Number(distance.toFixed(1))} {unit}
                      </strong>
                    </div>
                    <span>
                      {unit === 'km' ? 40 : 25} {unit}
              ...
              <div className="form-actions join-action-row">
                ...
                >
                  {isBuilding
                    ? t('night.builder.building')
                    : t('night.builder.buildAction')}
                </button>
              </div>
            ...
            {status ? (
              <div className="status-message compact-status">{status}</div>
            ) : null}

            {session && (
              <div className="night-ride-result-card glass-card animation-slide-up">
                <h3 className="card-title">
                  {session.crew_name || session.title}
                </h3>
                ...
                <div className="form-actions">
                  <button
                    ...
                    type="button"
                    onClick={() => openMapsUrl(session.route_url)}
                  >
                    {t('loop.openMaps')}
                  </button>
                  <button
                    className="secondary-button night-ride-post-button"
                    type="button"
        ...
        >
          <div className="modal-card animation-slide-up">
            <div className="modal-header">
              <div className="modal-title" id="night-post-modal-title">
                {t('night.modal.title')}
              </div>
              <button
            ...
            <div className="modal-body form-card">
              <div className="form-section section-block">
                ...
                {postStatus && (
                  <div className="status-message">{postStatus}</div>
                )}
              </div>
              <div className="form-actions">
                <button
                ...
                >
                  {isPosting
                    ? t('night.modal.posting')
                    : t('night.modal.postAction')}
                </button>
                <button
      ...
      {showCodeModal && (
        ...
        >
          <div className="modal-card animation-slide-up messenger-code-modal night-ride-code-modal">
            <div className="modal-header">
              <div className="modal-title" id="night-code-modal-title">
                {t('night.builder.haveCode')}
              </div>
              <button
                className="modal-close"
            ...
            <div className="modal-body form-card">
              ...
              <div className="form-actions">
                ...
                >
                  {isJoining
                    ? t('night.builder.joining')
                    : t('night.builder.loadCode')}
                </button>
                <button
                  className="ghost-button"
                  type="button"
