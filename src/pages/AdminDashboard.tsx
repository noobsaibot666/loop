...
const AdminDashboard: React.FC = () => {
  ...
  const [riderDrafts, setRiderDrafts] = useState<
    Record<string, {credits: string; free_used: string}>
  >({});
  ...
  const fetchRiders = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{riders: AdminRider[]}>(
        '/api/admin/rider-list',
        {},
      );
      const nextRiders = data.riders || [];
      setRiders(nextRiders);
      setRiderDrafts(
        nextRiders.reduce<Record<string, {credits: string; free_used: string}>>(
          (acc, rider) => {
            acc[rider.user_id] = {
  ...
  const fetchNightPosts = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{posts: AdminNightPost[]}>(
  ...
  const fetchProofs = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{proofs: AdminProof[]}>(
        '/api/admin/proofs',
        {},
      );
  ...
  const fetchPacks = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{packs: AdminCityPack[]}>(
  ...
  const fetchCheckpoints = async (packId: string) => {
    ...
    setIsLoading(true);
    try {
      const data = await postJSON<{checkpoints: AdminCityCheckpoint[]}>(
        '/api/admin/city-checkpoints',
        {pack_id: packId},
      );
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{requests: AdminCityRequest[]}>(
        '/api/admin/city-requests',
        {},
      );
  ...
  const fetchCollaborations = async () => {
    setIsLoading(true);
    try {
      const data = await postJSON<{requests: AdminCollaborationRequest[]}>(
      ...
      setCollaborationDrafts(
        nextRequests.reduce<Record<string, {status: string}>>(
          (acc, request) => {
            acc[request.user_id] = {
  ...
  useEffect(() => {
    async function checkAdmin() {
      ...
      try {
        const data = await postJSON<{is_admin: boolean}>(
          '/api/admin/check',
          {},
        );
  ...
  const filteredRiders = useMemo(() => {
    ...
    return riders.filter(rider =>
      [rider.email, rider.rider_name, rider.user_id].some(value =>
        String(value || '')
          .toLowerCase()
          .includes(query),
      ),
  ...
  const handleNightModeration = async (
    postId: string,
    moderationStatus: 'live' | 'flagged' | 'hidden',
  ) => {
    try {
      pushStatus('info', t('admin.messages.updating'));
      await postJSON('/api/admin/night-ride-moderation', {
        post_id: postId,
        moderation_status: moderationStatus,
      });
      pushStatus(
        'success',
        t('admin.messages.nightUpdated', {status: moderationStatus}),
      );
  ...
  const handleProofVisibility = async (proofId: string, isPublic: boolean) => {
    try {
      pushStatus('info', t('admin.messages.updating'));
      await postJSON('/api/admin/proof-visibility', {
        proof_id: proofId,
        is_public: isPublic,
      });
      pushStatus(
        'success',
        isPublic
          ? t('admin.messages.proofLive')
          : t('admin.messages.proofHidden'),
      );
      await fetchProofs();
  ...
  const handleArchiveMonth = async () => {
    ...
    try {
      pushStatus('info', t('admin.messages.updating'));
      const data = await postJSON<{archived: number}>(
        '/api/admin/proof-archive-month',
        {month: archiveMonth},
      );
      pushStatus(
        'success',
        t('admin.messages.archiveDone', {
  ...
  const handlePreviewManifest = async () => {
    try {
      pushStatus('info', t('admin.messages.updating'));
      const data = await postJSON<{manifest: PreviewManifest}>(
        '/api/admin/preview-manifest',
        {
    ...
    try {
      ...
      if (data.draft) {
        setPackForm(current => ({
          ...
          finish_label: String(
            data.draft.finish_label || current.finish_label || '',
          ),
        }));
      }
  ...
  const handleRequestDraft = async (requestId: string) => {
    try {
      pushStatus('info', t('admin.messages.updating'));
      const data = await postJSON<{draft?: any; pack?: AdminCityPack}>(
        '/api/admin/city-requests',
        {
          action: 'ai_draft',
          request_id: requestId,
  ...
  const isRetryableCommunityEvent = (eventType: string) =>
    [
      'discord_link_failed',
      'email_activation_failed',
      'email_discord_linked_failed',
      'email_cancellation_failed',
  ...
  if (!isAdmin) {
    return (
      <div className="status-message error page-loader">
    ...
    <div className="sequential-layout sub-page page-admin page-stage-enter">
      <Hero
        ...
        subtitle={t('admin.subtitle', {
          email: overview?.admin_email || user?.email || '',
        })}
        image={adminHero}
        actions={
          <div className="hero-actions">
            <button
              className={`pill ${activeTab === 'metrics' ? 'active' : ''}`}
              onClick={() => setActiveTab('metrics')}
            >
              {t('admin.tabs.metrics')}
            </button>
            <button
              className={`pill ${activeTab === 'riders' ? 'active' : ''}`}
              onClick={() => setActiveTab('riders')}
            >
              {t('admin.tabs.riders')}
            </button>
            <button
              className={`pill ${activeTab === 'night' ? 'active' : ''}`}
              onClick={() => setActiveTab('night')}
            >
              {t('admin.tabs.night')}
            </button>
            <button
              className={`pill ${activeTab === 'proofs' ? 'active' : ''}`}
              onClick={() => setActiveTab('proofs')}
            >
              {t('admin.tabs.proofs')}
            </button>
            <button
              className={`pill ${activeTab === 'packs' ? 'active' : ''}`}
              onClick={() => setActiveTab('packs')}
            >
              {t('admin.tabs.packs')}
            </button>
            <button
              className={`pill ${activeTab === 'requests' ? 'active' : ''}`}
              onClick={() => setActiveTab('requests')}
            >
              {t('admin.tabs.requests')}
            </button>
            <button
              className={`pill ${activeTab === 'collaboration' ? 'active' : ''}`}
      ...
      <section className="builder-grid single reveals">
        <div className="account-shell">
          ...
          {actionStatus && (
            <div className={`status-message ${actionStatus.tone}`}>
              {actionStatus.text}
            </div>
          )}
        </div>
          ...
          <div className="account-grid">
            <div className="glass-card form-card account-stats-card">
              <div className="form-header">
                ...
                <div className="achievement-badge gold">
                  <Crown size={14} /> {overview.admin_email}
                </div>
              </div>
                ...
                {overview.quarter.leaders.map(leader => (
                  <div key={leader.user_id} className="history-row">
                    <div>
                      ...
                      <span>
                        {leader.public_proofs} proofs · {leader.finished_runs}{' '}
                        finishes
                      </span>
                    </div>
            ...
            <div className="glass-card form-card account-purchases-card">
              <div className="form-header">
                <div>
                  <div className="form-title">
                    {t('admin.metrics.sessions')}
                  </div>
                  <div className="form-subtitle">
                    {t('admin.metrics.sessionsSubtitle')}
                  </div>
                </div>
            ...
            <div className="glass-card form-card account-purchases-card">
              <div className="form-header">
                <div>
                  ...
                  <div className="form-subtitle">
                    City, manifest, checkpoint bucket, and finish time.
                  </div>
                </div>
                <Zap size={18} className="text-muted" />
              </div>
                ...
                {overview.fastest_runs.map(entry => (
                  <div key={entry.run_id} className="history-row">
                    <div>
                      ...
                      <span>
                        {entry.city_name} · {entry.manifest_title}
                      </span>
                      <span>
                        {entry.checkpoint_count
                          ? `${entry.checkpoint_count} CP`
                          : '--'}{' '}
                        ·{' '}
                        {entry.ghost_seconds
                          ? `${Math.floor(entry.ghost_seconds / 60)}m ghost`
                          : 'no ghost'}
                      </span>
                      <span>
                        {entry.bike_name
                          ? `${entry.bike_name} · ${entry.bike_ratio || '--'}`
                          : 'bike not set'}
                      </span>
                    </div>
                    <div className="history-actions">
                      <strong>
                        {Math.floor(entry.finish_seconds / 60)}m{' '}
                        {String(entry.finish_seconds % 60).padStart(2, '0')}s
                      </strong>
                    </div>
                  </div>
                ))}
            ...
            <div className="glass-card form-card account-purchases-card">
              <div className="form-header">
                <div>
                  ...
                  <div className="form-subtitle">
                    Membership, Discord, and email state changes.
                  </div>
                </div>
                ...
                {(overview.community_events || []).map(event => (
                  <div key={event.id} className="history-row">
                    ...
                    <div className="history-actions">
                      <strong>
                        {event.details?.email ||
                          event.details?.discord_username ||
                          '--'}
                      </strong>
            ...
            <div className="glass-card form-card account-purchases-card">
              ...
              <div className="history-list">
                {filteredRiders.map(rider => (
                  <div
                    key={rider.user_id}
                    className="history-row admin-rider-row"
                  >
                    <div>
                    ...
                    <div className="admin-rider-controls">
                          ...
                          onChange={event =>
                            setRiderDrafts(current => ({
                              ...
                              [rider.user_id]: {
                                ...
                                free_used:
                                  current[rider.user_id]?.free_used || '0',
                              },
                            }))
                          }
                        />
                      ...
                      <div className="admin-action-row">
                        <button
                          type="button"
                          className="ghost-button small"
                          onClick={() => handleResetRider(rider.user_id)}
                        >
                        ...
                        <button
                          ...
                          onClick={() => handleSetCredits(rider.user_id)}
                        >
                          {t('admin.riders.set')}
                        </button>
                      </div>
              ...
              <div className="history-list">
                {!nightPosts.length ? (
                  ...
                  nightPosts.map(post => (
                    <div
                      key={post.id}
                      className="history-row admin-moderation-row"
                    >
                      <div className="admin-proof-main">
                        <img
                          src={post.image_url}
                          className="admin-thumb"
                          alt={post.route_title || post.rider_name}
                        />
                        <div>
                          <strong>
                            {post.route_title ||
                          ...
                          <span>
                            {post.rider_name} · {post.city_name || '--'} ·{' '}
                            {formatDate(post.created_at)}
                          </span>
                          <span>
                            {post.caption || t('admin.night.noCaption')}
                          </span>
                        </div>
                      </div>
                      <div className="admin-action-row">
                        <button
                          type="button"
                          className="ghost-button small"
                          onClick={() => handleNightModeration(post.id, 'live')}
                        >
                          {t('admin.night.live')}
                        </button>
                        <button
                          ...
                          className="ghost-button small"
                          onClick={() =>
                            handleNightModeration(post.id, 'flagged')
                          }
                        >
                          {t('admin.night.flag')}
                        </button>
                        <button
                          ...
                          className="ghost-button small"
                          onClick={() =>
                            handleNightModeration(post.id, 'hidden')
                          }
                        >
                          {t('admin.night.hide')}
                        </button>
                    ...
                    >
                      <div className="admin-proof-main">
                        ...
                        <div>
                          ...
                          <span>
                            {proof.city_name} · {proof.checkpoint_name}
                          </span>
                      ...
                      <div className="admin-action-row">
                        <button
                          ...
                          className="ghost-button small"
                          onClick={() =>
                            handleProofVisibility(proof.id, !proof.is_public)
                          }
                        >
                          {proof.is_public ? (
                            <EyeOff size={14} />
                        ...
                        <button
                          type="button"
                          className="ghost-button small"
                          onClick={() => handleProofDelete(proof.id)}
                        >
                          <Trash2 size={14} />
          ...
          <div className="account-grid admin-pack-grid">
            <div className="glass-card form-card account-purchases-card">
              ...
              <label className="field">
                ...
                <textarea
                  value={packForm.route_note}
                  onChange={event =>
                    setPackForm(current => ({
                      ...current,
                      route_note: event.target.value,
                    }))
                  }
                />
              </label>
              <label className="field">
                ...
                <input
                  value={packForm.finish_label}
                  onChange={event =>
                    setPackForm(current => ({
                      ...current,
                      finish_label: event.target.value,
                    }))
                  }
                />
              </label>
              <label className="field">
                ...
                <textarea
                  value={packForm.safety_note}
                  onChange={event =>
                    setPackForm(current => ({
                      ...current,
                      safety_note: event.target.value,
                    }))
                  }
                />
              </label>
              ...
              <div className="admin-action-row admin-action-row-left">
                ...
                <button
                  ...
                  disabled={!selectedPackId && !packForm.name.trim()}
                >
              ...
              <div className="admin-preview-controls">
                ...
                <label className="field admin-mini-field">
                  ...
                  <input
                    value={previewState.checkpoint_count}
                    onChange={event =>
                      setPreviewState(current => ({
                        ...current,
                        checkpoint_count: event.target.value,
                      }))
                    }
                  />
                </label>
              ...
              {previewManifest && (
                <div className="glass-card admin-preview-card">
                  ...
                  <div className="admin-preview-metrics">
                    <span>
                      {t('admin.packs.checkpointCount')}:{' '}
                      {previewManifest.checkpoint_count || 0}
                    </span>
                    <span>
                      {t('admin.preview.eta')}:{' '}
                      {previewManifest.estimated_minutes || 0}m
                    </span>
                    <span>
                      {t('alleycat.ghostRider')}:{' '}
                      {previewManifest.ghost_label || t('common.off')}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="glass-card form-card account-purchases-card">
              <div className="form-header">
                <div>
                  <div className="form-title">
                    {t('admin.checkpoints.title')}
                  </div>
                  <div className="form-subtitle">
                    {t('admin.checkpoints.subtitle')}
                  </div>
                </div>
                <Zap size={18} className="text-muted" />
              </div>
              <div className="profile-grid">
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.slug || '')}
                    onChange={event =>
                ...
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.name || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
                        name: event.target.value,
              ...
              <div className="profile-grid">
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.lat || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
                ...
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.lng || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
              ...
              <div className="profile-grid">
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.district || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
                ...
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.category || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
              ...
              <div className="profile-grid">
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.vibe || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
                ...
                <label className="field">
                  ...
                  <input
                    value={String(checkpointForm.sort_weight || '')}
                    onChange={event =>
                      setCheckpointForm(current => ({
                        ...current,
              ...
              <label className="field">
                ...
                <textarea
                  value={String(checkpointForm.hint || '')}
                  onChange={event =>
                    setCheckpointForm(current => ({
                      ...current,
              ...
              <label className="field">
                ...
                <textarea
                  value={String(checkpointForm.task_local || '')}
                  onChange={event =>
                    setCheckpointForm(current => ({
                      ...current,
                      task_local: event.target.value,
                    }))
                  }
                />
              </label>
              <label className="field">
                ...
                <textarea
                  value={String(checkpointForm.task_fast || '')}
                  onChange={event =>
                    setCheckpointForm(current => ({
                      ...current,
                      task_fast: event.target.value,
                    }))
                  }
                />
              </label>
              <label className="field">
                ...
                <textarea
                  value={String(checkpointForm.task_chaotic || '')}
                  onChange={event =>
                    setCheckpointForm(current => ({
                      ...current,
                      task_chaotic: event.target.value,
                    }))
                  }
                />
              </label>
              ...
              <div className="history-list">
                {!requests.length ? (
                  ...
                  requests.map(request => (
                    <div
                      key={request.id}
                      className="history-row admin-request-row"
                    >
                      <div>
                        <strong>
                        ...
                        <span>
                          {request.rider_name || request.email || '--'} ·{' '}
                      ...
                      <div className="admin-request-controls">
                          ...
                          <input
                            ...
                            onChange={event =>
                              setRequestDrafts(current => ({
                                ...
                                [request.id]: {
                                  status:
                                    current[request.id]?.status || 'reviewing',
                        ...
                        <div className="admin-action-row">
                          <button
                            type="button"
                            className="ghost-button small"
                            onClick={() => handleRequestDraft(request.id)}
                          >
                            <Sparkles size={14} />
          ...
          <div className="account-grid">
            <div className="glass-card form-card account-purchases-card">
              <div className="form-header">
                <div>
                  <div className="form-title">
                    {t('admin.collaboration.title')}
                  </div>
                  <div className="form-subtitle">
                    {t('admin.collaboration.subtitle')}
                  </div>
                </div>
                <Users size={18} className="text-muted" />
              </div>
                    ...
                    >
                      <div>
                        ...
                        <span>
                          ...
                          {formatDate(
                            request.collaboration_requested_at ||
                              request.updated_at,
                          )}
                        </span>
                      ...
                      <div className="admin-request-controls">
                        <label className="field admin-mini-field">
                          ...
                          <select
                            value={
                              collaborationDrafts[request.user_id]?.status ||
                              'pending'
                            }
                            onChange={event =>
                          ...
                          >
                            <option value="pending">
                              {t('admin.collaboration.pending')}
                            </option>
                            <option value="reviewing">
                              {t('admin.collaboration.reviewing')}
                            </option>
                            <option value="approved">
                              {t('admin.collaboration.approved')}
                            </option>
                            <option value="rejected">
                              {t('admin.collaboration.rejected')}
                            </option>
                          </select>
                        </label>
                        <div className="admin-action-row">
                          <button
                            ...
                            className="primary-button small"
                            onClick={() =>
                              handleCollaborationUpdate(request.user_id)
                            }
                          >
