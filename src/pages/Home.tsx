...
const Home: React.FC = () => {
  ...
  const [communityBusy, setCommunityBusy] = React.useState<
    'checkout' | 'invite' | 'connect' | 'portal' | null
  >(null);
  ...
  const [badgeCities, setBadgeCities] = React.useState<string[]>([
    'Curitiba/BR',
    'Munich/DE',
    'Guarulhos/BR',
  ]);
  const [currentBadgeIndex, setCurrentBadgeIndex] = React.useState(0);
  const [leavingBadgeIndex, setLeavingBadgeIndex] = React.useState<
    number | null
  >(null);
  ...
  React.useEffect(() => {
    const hideTimer = window.setTimeout(
      ...
      badgeHoldDurationMs * badgeCities.length + badgeExitDelayMs,
    );
  ...
  return (
    <div className="sequential-layout page-home page-stage-enter">
      <Hero
        ...
        badge={
          <div
            className={`hero-badge-pill ${badgeVisible ? 'pill-appear' : 'pill-hide'}`}
          >
            <span className="badge-tag">{t('cities.newCity')}</span>
            <span className="badge-label-viewport" aria-live="polite">
              {leavingBadgeIndex !== null ? (
                <span className="badge-label badge-label-leaving badge-label-out">
                  {badgeCities[leavingBadgeIndex]}
                </span>
              ) : null}
              <span
                className={`badge-label badge-label-current ${leavingBadgeIndex !== null ? 'badge-label-in' : ''}`}
              >
      ...
      <section className="modular-grid home-modular-grid reveals">
        ...
        >
          ...
          <span className="ghost-button small home-card-button">
            {t('home.loop.action')}
          </span>
        </div>
        <div
          ...
          style={
            {'--home-card-image': `url(${alleycatCardHero})`} as CSSProperties
          }
        ...
        <div
          ...
          style={
            {'--home-card-image': `url(${nightRideCardHero})`} as CSSProperties
          }
          {...activateCard('/night')}
        >
          <div className="home-card-title-row">
      ...
      <section className="modular-grid reveals home-lower-grid">
        <div className="modular-cell home-community-card home-community-card-discord">
          ...
          <div className="home-community-brand-footer" aria-hidden="true">
            <img
              className="home-community-brand-mark home-community-brand-mark-large"
              src={discordLogo}
              alt=""
            />
          </div>
        ...
        <div className="modular-cell modular-cell-community home-community-card home-community-card-strava">
          <div className="home-card-title-row">
            ...
            <span className="home-card-pill">
              {t('home.community.strava.pill')}
            </span>
          </div>
          <p className="cell-body home-community-strava-body">
            {t('home.community.strava.body')}
          </p>
          <a
      ...
      {communityModalOpen ? (
        ...
        >
          <div
            className="modal-card community-membership-modal animation-slide-up"
            onClick={event => event.stopPropagation()}
          >
            <div className="modal-header">
              <div>
                <div className="modal-title">
                  {t('account.community.heroTitle')}
                </div>
                <div className="modal-subtitle">
                  {t('account.community.heroSubtitle')}
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="modal-body">
              ...
              <div className="account-note community-membership-note">
                {t('account.community.modalNote')}
              </div>
              {communityError ? (
                <div className="status-line error">{communityError}</div>
              ) : null}
