...
const getBikeSlotLabel = (index: number) =>
  `Bike ${String.fromCharCode(65 + index)}`;
...
const RiderAccount: React.FC = () => {
  ...
  const {usage, fetchUsage, fetchAccountSummary, accountSummary, isLoading} =
    useCreditStore();
  ...
  const [membershipBusy, setMembershipBusy] = useState<
    'checkout' | 'invite' | 'portal' | 'connect' | null
  >(null);
  const [creditCheckoutAmount, setCreditCheckoutAmount] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (communityState === 'discord-linked')
      pushStatus('success', t('account.messages.discordLinked'));
    if (communityState === 'discord-denied')
      pushStatus('info', t('account.messages.discordDenied'));
    if (communityState === 'discord-expired')
      pushStatus('error', t('account.messages.discordExpired'));
    if (communityState === 'discord-error')
      pushStatus('error', t('account.messages.discordError'));
    if (communityState === 'discord-inactive')
      pushStatus('error', t('account.messages.discordInactive'));
    if (accessToken) void fetchAccountSummary(accessToken);
    ...
    async function loadNightRides() {
      ...
      try {
        const res = await getJSON<
          {sessions: NightRideAccountSession[]} | NightRideAccountSession[]
  ...
  const pushStatus = (tone: StatusTone, text: string) =>
  ...
  const isMembershipActive = Boolean(
    membership?.access_active || membership?.access_state === 'active',
  );
  ...
  const accountName =
    accountSummary?.profile?.rider_name ||
    user?.email?.split('@')[0] ||
    'rider';
  const greeting = t('account.greeting', {name: accountName});
  const accountSubtitle = user
    ? t('account.subtitleAuthed', {greeting})
    : t('account.subtitleGuest');
  ...
  const visiblePurchases = expanded.purchases
    ? accountSummary?.purchases || []
    : (accountSummary?.purchases || []).slice(0, 4);
  const visibleLoops = expanded.loops
    ? accountSummary?.loop_history || []
    : (accountSummary?.loop_history || []).slice(0, 4);
  const visibleHunts = expanded.hunts
    ? accountSummary?.alleycat_history || []
    : (accountSummary?.alleycat_history || []).slice(0, 4);
  const visibleChallenges = expanded.challenges
    ? accountSummary?.challenge_history || []
    : (accountSummary?.challenge_history || []).slice(0, 4);
  const visibleCrew = expanded.crew
    ? accountSummary?.shared_riders || []
    : (accountSummary?.shared_riders || []).slice(0, 4);
  ...
  const updateBike = (
    bikeId: string,
    field: 'bike_name' | 'bike_ratio',
    value: string,
  ) =>
    setBikes(current =>
      normalizeBikeList(
        current.map(bike =>
          bike.id === bikeId ? {...bike, [field]: value} : bike,
        ),
      ),
    );
  const setDefaultBike = (bikeId: string) =>
    setBikes(current =>
      normalizeBikeList(
        current.map(bike => ({...bike, is_default: bike.id === bikeId})),
      ),
    );
  const removeBike = (bikeId: string) =>
    setBikes(current =>
      normalizeBikeList(current.filter(bike => bike.id !== bikeId)),
    );
  ...
  const handleOpenInvite = async () => {
    try {
      ...
      const {url} = await postJSON<{url: string}>(
        '/api/community-membership/access',
        {},
      );
      window.location.href = url;
    } catch (error: any) {
  ...
  const handleConnectDiscord = async () => {
    try {
      ...
      const {url} = await postJSON<{url: string}>(
        '/api/community-membership/discord-start',
        {},
      );
      window.location.href = url;
    } catch (error: any) {
  ...
  const handleFeedbackSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await postJSON('/api/account-feedback', {
        feedback,
        rider_name:
          profileForm.rider_name || accountSummary?.profile?.rider_name || '',
      });
      setFeedback('');
      ...
      pushStatus(
        'error',
        error.message || t('account.messages.feedbackFailed'),
      );
    } finally {
    ...
    <div className="sequential-layout sub-page page-account page-stage-enter">
      ...
      <section className="builder-grid single reveals">
          ...
          <div className="account-topbar">
            <div>
              ...
              <div className="account-identity">
                <strong>
                  {accountSummary?.profile?.rider_name || accountName}
                </strong>
                <span>{user?.email || t('account.profile.noEmail')}</span>
              </div>
            </div>
            <div className="section-jump-strip account-topbar-actions">
              <a className="mini-chip active" href="#account-setup">
                {t('account.jump.setup')}
              </a>
              <a className="mini-chip" href="#account-credits">
                {t('account.jump.credits')}
              </a>
              <a className="mini-chip" href="#account-stats">
                {t('account.jump.stats')}
              </a>
              <a className="mini-chip" href="#account-history">
                {t('account.jump.history')}
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="account-grid">
          <div
            className="glass-card form-card account-summary-card"
            id="account-setup"
          >
            <div className="form-header">
              <div>
                ...
                <label className="field">
                  ...
                  <input
                    ...
                    onChange={event =>
                      setProfileForm(current => ({
                        ...current,
                        home_location: event.target.value,
                      }))
                    }
                  />
                </label>
              ...
              <div className="account-bike-block">
                <div className="form-header account-inline-header">
                  <div>
                    <div className="section-title small-title">
                      {t('account.profile.bikesTitle')}
                    </div>
                    <div className="form-subtitle">
                      {t('account.profile.bikesSubtitle')}
                    </div>
                  </div>
                  <button
                    type="button"
                ...
                <div className="account-bike-list">
                  {bikes.map((bike, index) => (
                    <div
                      key={bike.id}
                      className={`account-bike-card ${bike.is_default ? 'active' : ''}`}
                    >
                      <div className="account-bike-head">
                        <div className="account-bike-title">
                          <span className="account-bike-slot-pill">
                            {getBikeSlotLabel(index)}
                          </span>
                          <strong>
                            {bike.bike_name.trim() ||
                              t('account.profile.bikeFallback', {
                                number: index + 1,
                              })}
                          </strong>
                        </div>
                        <div className="account-bike-actions">
                          ...
                          >
                            {bike.is_default
                              ? t('account.profile.defaultBike')
                              : t('account.profile.makeDefault')}
                          </button>
                          <button
                            type="button"
                            className="ghost-button small account-card-reset"
                      ...
                      <div className="profile-grid">
                        <label className="field">
                          ...
                          <input
                            ...
                            placeholder={t(
                              'account.profile.bikeNamePlaceholder',
                            )}
                            onChange={event =>
                              updateBike(
                                bike.id,
                        ...
                        <label className="field">
                          ...
                          <input
                            ...
                            placeholder={t(
                              'account.profile.bikeRatioPlaceholder',
                            )}
                            onChange={event =>
                              updateBike(
                                bike.id,
                ...
                <button
                  ...
                  onClick={() => {
                    setProfileForm({
                      rider_name: accountSummary?.profile?.rider_name || '',
                      home_location:
                        accountSummary?.profile?.home_location || '',
                      bike_name: accountSummary?.profile?.bike_name || '',
                      bike_ratio: accountSummary?.profile?.bike_ratio || '',
                      collaboration_note:
                        accountSummary?.profile?.collaboration_note || '',
                    });
                    setBikes(
                      (accountSummary?.bikes || []).length
                        ? accountSummary?.bikes || []
                        : accountSummary?.profile?.bike_name ||
                            accountSummary?.profile?.bike_ratio
                          ? [
                              {
                                id: 'legacy-bike',
                                bike_name:
            ...
            <div className="account-stack">
              <div>
                ...
                <div className="form-subtitle">
                  {user.email || t('account.profile.noEmail')}
                </div>
              </div>
              <div className="profile-grid">
                <label className="field">
          ...
          <div
            className="glass-card form-card account-credits-card"
            id="account-credits"
          >
            <div className="form-header">
              <div>
                ...
                <div className="form-subtitle">
                  {t('account.credits.subtitle')}
                </div>
              </div>
              {usage?.is_admin && (
                <div className="achievement-badge gold">
                  <Crown size={14} /> {t('credits.admin')}
                </div>
              )}
            </div>
          ...
          <div className="glass-card form-card account-community-card">
            <div className="form-header">
              <div>
                ...
                <div className="form-subtitle">
                  {t('account.community.subtitle')}
                </div>
              </div>
              <div
                className={`mini-chip ${isMembershipActive ? 'active' : ''}`}
              >
                {membership?.access_state ||
                  membership?.status ||
                  t('account.community.statusInactive')}
              </div>
            </div>
            ...
            <div className="result-grid result-grid-two account-credit-grid">
              ...
              <div className="metric-group">
                ...
                <strong>
                  {membership
                    ? formatMoney(membership.price_cents, membership.currency)
                    : '--'}
                </strong>
              </div>
              <div className="metric-group">
            ...
            {discordLinked && membership?.discord_username ? (
              <div className="account-note">
                {t('account.community.linkedAs', {
                  name: membership.discord_username,
                })}
              </div>
            ) : null}
            {isMembershipActive && !discordReady ? (
              <div className="account-note">
                {t('account.community.linkNeeded')}
              </div>
            ) : null}
            {membership?.current_period_end && (
              <div className="account-note">
                {t('account.community.renews', {
            ...
            <div className="form-actions">
              {!isMembershipActive && (
                <button
                  type="button"
                  className="primary-button"
                  disabled={membershipBusy === 'checkout'}
              ...
              {isMembershipActive && !discordReady && (
                <button
                  type="button"
                  className="primary-button"
                  disabled={membershipBusy === 'connect'}
                  onClick={handleConnectDiscord}
                >
                  {membershipBusy === 'connect'
              ...
              {isMembershipActive && discordReady && (
                <button
                  type="button"
                  className="primary-button"
                  disabled={membershipBusy === 'invite'}
                  onClick={handleOpenInvite}
                >
                  {membershipBusy === 'invite'
              ...
              {membership && (
                <button
                  type="button"
                  className="ghost-button account-community-secondary-action"
                  disabled={membershipBusy === 'portal'}
                  onClick={handleManagePortal}
                >
                  {membershipBusy === 'portal'
                    ? t('common.working')
                    : t('account.community.manageBilling')}
                </button>
              )}
            </div>
          </div>
          ...
          <div
            className="glass-card form-card account-stats-card"
            id="account-stats"
          >
            <div className="form-header">
              <div>
          ...
          <div className="glass-card form-card account-quarter-card">
            <div className="form-header">
              <div>
                ...
                <div className="form-subtitle">
                  {t('account.quarter.subtitle', {
                    label: quarter?.label || '--',
                  })}
                </div>
              </div>
              <div className="achievement-badge gold">
                <Trophy size={14} /> {quarter?.label || '--'}
              </div>
            </div>
            <div className="result-grid result-grid-three">
              <div className="metric-group">
            ...
            <div className="account-note">
              {quarter?.total_ranked_riders
                ? t('account.quarter.ridersOnBoard', {
                    count: quarter.total_ranked_riders,
                  })
                    ...
                    <div>
                      <strong>
                        <button
                          ...
                          onClick={() => navigate(`/rider/${leader.user_id}`)}
                        >
                          {leader.rider_name}
                        </button>
                      </strong>
                      <span>
                        {t('account.quarter.proofsFinishes', {
                          proofs: leader.public_proofs,
                          finishes: leader.finished_runs,
                        })}
                      </span>
                    </div>
                ...
                <div className="purchase-list">
                  {visiblePurchases.map(purchase => (
                    <div key={purchase.session_id} className="purchase-row">
                      ...
                      <div className="history-actions">
                        <strong>
                          {t('account.purchases.credits', {
                            count: purchase.credits_to_grant,
                          })}
                        </strong>
          ...
          <div
            className="glass-card form-card account-history-card"
            id="account-history"
          >
            <div className="form-header">
              <div>
                <div className="form-title">
                  {t('account.loopHistory.title')}
                </div>
                <div className="form-subtitle">
                  {t('account.loopHistory.subtitle')}
                </div>
              </div>
              ...
              <>
                  ...
                  {visibleLoops.map(route => (
                    <div key={route.id} className="history-row">
                      <div>
                        ...
                        <span>
                          {route.distance_km} km · {route.surface} ·{' '}
                          {route.vibe}
                        </span>
                        {route.bike_name ? (
                          <span>
                            {t('common.bikeUsed')}: {route.bike_name} ·{' '}
                            {route.bike_ratio || '--'}
                          </span>
                        ) : null}
                      </div>
                ...
                {accountSummary.loop_history.length > 4 && (
                  <button
                    type="button"
                    className="ghost-button small"
                    onClick={() => toggleExpanded('loops')}
                  >
                    {expanded.loops
                      ? t('common.showLess')
                      : t('account.loopHistory.showMore')}
                  </button>
                )}
              </>
            )}
          ...
          <div className="glass-card form-card account-history-card">
            <div className="form-header">
              <div>
                <div className="form-title">
                  {t('account.alleycatHistory.title')}
                </div>
                <div className="form-subtitle">
                  {t('account.alleycatHistory.subtitle')}
                </div>
              </div>
            ...
            {!accountSummary?.alleycat_history?.length ? (
              <div className="account-note">
                {t('account.alleycatHistory.empty')}
              </div>
            ) : (
              <>
                <div className="history-list">
                  {visibleHunts.map(hunt => (
                    <div key={hunt.id} className="history-row">
                      <div>
                        ...
                        <span>
                          {hunt.city_name} · {hunt.difficulty} · {hunt.style}
                        </span>
                        <span>
                          {hunt.best_seconds
                            ? formatDuration(hunt.best_seconds)
                            : '--:--'}{' '}
                          ·{' '}
                          {t('account.alleycatHistory.proofs', {
                            count: hunt.proof_count,
                          })}
                        </span>
                        {hunt.proofs?.length ? (
                      ...
                      <div className="history-actions">
                        <strong>
                          {hunt.source_challenge_id
                            ? t('account.alleycatHistory.shared')
                            : t('account.alleycatHistory.solo')}
                        </strong>
                ...
                {accountSummary.alleycat_history.length > 4 && (
                  <button
                    type="button"
                    className="ghost-button small"
                    onClick={() => toggleExpanded('hunts')}
                  >
                    {expanded.hunts
                      ? t('common.showLess')
                      : t('account.alleycatHistory.showMore')}
                  </button>
                )}
              </>
            )}
          ...
          <div className="glass-card form-card account-history-card">
            <div className="form-header">
              <div>
                <div className="form-title">
                  {t('account.challenges.title')}
                </div>
                <div className="form-subtitle">
                  {t('account.challenges.subtitle')}
                </div>
              </div>
              ...
              <>
                <div className="history-list">
                  {visibleChallenges.map(challenge => (
                    <div key={challenge.challenge_id} className="history-row">
                      <div>
                        <strong>
                          {challenge.manifest_title ||
                            t('account.challenges.manifestFallback')}
                        </strong>
                        <span>
                          {challenge.city_name} ·{' '}
                          {t('account.challenges.code', {code: challenge.code})}
                        </span>
                      </div>
                      <div className="history-actions">
                        <strong>
                          {challenge.best_seconds
                            ? formatDuration(challenge.best_seconds)
                            : '--:--'}
                        </strong>
                        <span>
                          {t('account.challenges.rivals', {
                            count: challenge.rival_count,
                          })}
                        </span>
                      </div>
                    </div>
                ...
                {accountSummary.challenge_history.length > 4 && (
                  <button
                    type="button"
                    className="ghost-button small"
                    onClick={() => toggleExpanded('challenges')}
                  >
                    {expanded.challenges
                      ? t('common.showLess')
                      : t('account.challenges.showMore')}
                  </button>
                )}
              </>
            )}
              ...
              <>
                      ...
                      <div>
                        <strong>
                          <button
                            type="button"
                            className="inline-link-button"
                            onClick={() => navigate(`/rider/${rider.user_id}`)}
                          >
                            {rider.rider_name}
                          </button>
                        </strong>
                        <span>
                          {t('account.crew.sharedChallenges', {
                            count: rider.shared_challenges,
                          })}
                        </span>
                        <span>
                          {rider.cities.length
                            ? rider.cities.join(' · ')
                            : t('account.crew.noCityTags')}
                        </span>
                      </div>
                      <div className="history-actions">
                ...
                {accountSummary.shared_riders.length > 4 && (
                  <button
                    type="button"
                    className="ghost-button small"
                    onClick={() => toggleExpanded('crew')}
                  >
                    {expanded.crew
                      ? t('common.showLess')
                      : t('account.crew.showMore')}
                  </button>
                )}
              </>
            )}
              ...
              <>
                  ...
                  {visibleNight.map(session => (
                    <div key={session.id} className="history-row">
                      <div>
                        ...
                        <span>
                          {session.ride_city || t('account.common.city')} ·{' '}
                          {session.mode} · {session.distance_km} km
                        </span>
                        {session.bike_name ? (
                          <span>
                            {t('common.bikeUsed')}: {session.bike_name} ·{' '}
                            {session.bike_ratio || '--'}
                          </span>
                        ) : null}
                        <span>
                          {session.crew_name || t('account.night.noCrewNames')}
                        </span>
                      </div>
                      <div className="history-actions">
                        <strong>
                ...
                {nightRides.length > 4 && (
                  <button
                    type="button"
                    className="ghost-button small"
                    onClick={() => toggleExpanded('night')}
                  >
                    {expanded.night
                      ? t('common.showLess')
                      : t('account.night.showMore')}
                  </button>
                )}
              </>
            )}
          ...
          <div className="glass-card form-card account-purchases-card">
            ...
            <form className="account-stack" onSubmit={handleFeedbackSubmit}>
              ...
              <div className="account-note">
                {t('account.feedback.wordCount', {
                  count: (feedback.trim().match(/\S+/g) || []).length,
                })}
              </div>
              <div className="form-actions account-form-actions">
                <button
          ...
          <div className="glass-card form-card account-danger-card">
            ...
            <div className="danger-item">
              <div className="danger-item-info">
                <div className="danger-item-title">
                  {t('account.dangerZone.exportTitle')}
                </div>
                <div className="danger-item-body">
                  {t('account.dangerZone.exportBody')}
                </div>
              </div>
              <div className="danger-actions">
                ...
                >
                  <Download size={16} />
                  {isExporting
                    ? t('account.dangerZone.exporting')
                    : t('account.dangerZone.exportAction')}
                </button>
              </div>
            </div>
            ...
            <div className="danger-item">
              <div className="danger-item-info">
                ...
                <div className="danger-item-body">
                  {t('account.dangerZone.deleteBody')}
                </div>
              </div>
              <div className="danger-actions">
                {showDeleteConfirm ? (
                  <>
                    ...
                    >
                      ...
                      {isDeleting
                        ...
                        : t('account.dangerZone.confirmDelete')}
                    </button>
                    <button
                      className="ghost-button small"
