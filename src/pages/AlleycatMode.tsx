...
const AlleycatMode: React.FC = () => {
  ...
  const [answerWrongCount, setAnswerWrongCount] = useState<
  ...
  const {usage, fetchUsage, accountSummary, fetchAccountSummary} =
    useCreditStore();
  ...
  useEffect(() => {
    ...
    const scrollTimer = window.setTimeout(() => {
      codeModalCardRef.current?.scrollIntoView({
        block: 'center',
        behavior: 'smooth',
      });
    }, 80);
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => {
      manifestResultRef.current?.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    }, 120);
  ...
  const rangePercent =
    ((config.range - minDistance) / (maxDistance - minDistance)) * 100;

  const handleUnitChange = (next: 'km' | 'mi') => {
    if (next === config.unit) return;
    const converted =
      next === 'km' ? config.range * 1.60934 : config.range / 1.60934;
  ...
  const steps = useMemo(
    () => [
      {
        number: '01',
        title: t('alleycat.step1.title'),
        body: t('alleycat.step1.body'),
      },
      {
        number: '02',
        title: t('alleycat.step2.title'),
        body: t('alleycat.step2.body'),
      },
      {
        number: '03',
        title: t('alleycat.step3.title'),
        body: t('alleycat.step3.body'),
      },
      {
        number: '04',
        title: t('alleycat.step4.title'),
        body: t('alleycat.step4.body'),
      },
    ],
    [t],
  );
  ...
  const formatBikeOptionLabel = (
    bike: (typeof riderBikes)[number],
    index: number,
  ) =>
  ...
  const finishedRiders = leaderboard.filter(
    entry => entry.best_seconds !== null,
  ).length;
  ...
  useEffect(() => {
    ...
    if (
      !selectedBikeId ||
      !riderBikes.some(bike => bike.id === selectedBikeId)
    ) {
      setSelectedBikeId(
        (riderBikes.find(bike => bike.is_default) || riderBikes[0]).id,
      );
    }
  ...
  const orderedCheckpoints = useMemo(() => {
    ...
    const checkpointMap = new Map(
      manifestCheckpoints.map(checkpoint => [checkpoint.id, checkpoint]),
    );
    ...
    const missing = manifestCheckpoints.filter(
      checkpoint => !checkpointOrder.includes(checkpoint.id),
    );
    return [...ordered, ...missing];
  }, [checkpointOrder, manifestCheckpoints]);
  const completedCheckpointIds = run?.completedIds || [];
  const checkinMap = run?.checkins || {};
  const completedCount = orderedCheckpoints.filter(checkpoint =>
  ...
  const riderElapsedSeconds = run
    ? (run.finishSeconds ??
      Math.max(
        0,
        Math.floor((comparisonNow - new Date(run.startedAt).getTime()) / 1000),
      ))
    : null;
  const ghostSplitSeconds = useMemo(() => {
    ...
    const totalSeconds =
      ghostTotalSeconds ??
      Math.max(60, (manifest?.estimated_minutes || 1) * 60);
    return orderedCheckpoints.map((_, index) =>
      Math.round(
        (totalSeconds * (index + 1)) / (orderedCheckpoints.length + 1),
      ),
    );
  ...
  const completedCheckpointSequence = useMemo(
    () =>
      Object.entries(checkinMap)
        .sort(
          (left, right) =>
            new Date(left[1]).getTime() - new Date(right[1]).getTime(),
        )
  ...
  const completionOrderMap = useMemo(
    () =>
      completedCheckpointSequence.reduce<Record<string, number>>(
  ...
  const riderCheckpointSeconds = (checkpointId: string) => {
    ...
    return Math.max(
      1,
      Math.round(
        (new Date(checkinMap[checkpointId]).getTime() -
          new Date(run.startedAt).getTime()) /
          1000,
      ),
    );
  };
  const riderProgress = totalCheckpoints
    ? (completedCount / totalCheckpoints) * 100
    : 0;
  const ghostProgress =
    totalCheckpoints && ghostSplitSeconds.length && riderElapsedSeconds !== null
      ? Math.min(
          100,
          (riderElapsedSeconds /
            (ghostTotalSeconds ?? (manifest?.estimated_minutes || 1) * 60)) *
            100,
        )
      : 0;
  const latestCompletedCheckpointId =
    completedCheckpointSequence[completedCheckpointSequence.length - 1] || null;
  const latestCompletedOrder = latestCompletedCheckpointId
    ? (completionOrderMap[latestCompletedCheckpointId] ?? null)
    : null;
  const latestCheckpointActualSeconds = latestCompletedCheckpointId
    ? riderCheckpointSeconds(latestCompletedCheckpointId)
    : null;
  const riderCheckpointTarget =
    latestCompletedOrder !== null
      ? ghostSplitSeconds[latestCompletedOrder]
      : null;
  const riderDeltaSeconds =
    latestCheckpointActualSeconds !== null && riderCheckpointTarget !== null
      ? latestCheckpointActualSeconds - riderCheckpointTarget
      : null;
  const ghostStyleLabel = t(
    `alleycat.style.${manifest?.style || config.style}`,
  );
  const ghostPressureLabel = t(
    `difficulty.${manifest?.difficulty || config.difficulty}`,
  );
  const allCheckpointsCleared =
    totalCheckpoints > 0 && completedCount >= totalCheckpoints;
  const nextGhostSplitSeconds =
    completedCount < ghostSplitSeconds.length
      ? ghostSplitSeconds[completedCount]
      : null;
  const activeCheckpoint = hasActiveRun
    ? orderedCheckpoints.find(
        checkpoint => !completedCheckpointIds.includes(checkpoint.id),
      ) || null
    : null;
  const manifestRangeLabel = manifest?.range_km
    ? `${manifest.range_km.toFixed(1)} km`
    : `${manifest?.estimated_minutes || 0} min`;
  const manifestStartLabel =
    manifest?.start_label || manifest?.city || config.city;
  const manifestDistrictCount =
    manifest?.district_count || manifest?.checkpoint_count || 0;
  const resolvedShareCode = String(shareCode || challenge?.code || '')
    .trim()
    .toUpperCase();
  useEffect(() => {
    if (
    ...
    const updateProximity = () => {
      navigator.geolocation.getCurrentPosition(
        position => {
          ...
          setNearCheckpointId(
            meters <= CHECKPOINT_HELP_RADIUS_METERS
              ? activeCheckpoint.id
              : null,
          );
        },
  ...
  const unlockCheckpointHint = async (
    checkpoint: NonNullable<(typeof orderedCheckpoints)[number]>,
  ) => {
  ...
  return (
    <div className="sequential-layout sub-page page-messenger page-stage-enter">
        ...
        actions={
          <div className="hero-actions-group">
            <button
              ...
              onClick={() =>
                document
                  .getElementById('manifest-killer')
        ...
        {steps.map(step => (
          <div key={step.number} className="module-card route-step-card">
            <div className="module-header route-step-header">
              <span className="module-number route-step-number">
                {step.number}
              </span>
      ...
      <section
        className="split-module reveals route-builder-section"
        id="manifest-killer"
      >
            ...
            <div className="form-header">
              ...
              <div className="loops-left">
                <span className="loops-left-line">
                  {hasUnlimitedCredits
                    ? t('alleycat.creditsUnlimited')
                    : t('credits.balance', {count: messengerCreditsOnly})}
                </span>
                <span className="loops-left-line">
                  {t('alleycat.perManifest', {count: MESSENGER_CREDIT_COST})}
                </span>
              </div>
            </div>
            ...
            <div className="form-body">
              <div className="form-section section-block-clean messenger-stack-section">
                ...
                <label className="field">
                  ...
                  >
                    ...
                    />
                    {isSuggesting && (
                      <div className="field-hint">{t('common.searching')}</div>
                    )}
              ...
              <div className="form-section section-block">
                <label className="field range-field">
                  ...
                  <div className="range-labels">
                    <span>
                      {minDistance} {config.unit}
                    </span>
                    <div className="range-focus-card">
                      <strong>
                        {config.range} {config.unit}
                      </strong>
                    </div>
                    <span>
                      {maxDistance} {config.unit}
                    </span>
                  </div>
                </label>
              </div>
              ...
              <div className="form-section section-block">
                <label className="field">
                  ...
                  <div className="field-kicker field-kicker-ghost">
                    {t('alleycat.ghostRiderIntro')}
                  </div>
      ...
      {manifest && (
        <section className="split-module reveals route-builder-section">
          <div className="module-content">
            <div
              ref={manifestResultRef}
              className="glass-card animation-slide-up manifest-result-panel"
              id="manifest-killer-result"
            >
              <div className="manifest-result-header">
                <div className="messenger-output">
                          ...
                          <label className="field manifest-bike-field">
                            ...
                            {riderBikes.length > 1 ? (
                              ...
                              <div className="account-note">
                                {t('common.standardBike')}: Bike A ·{' '}
                                {riderBikes[0].bike_name} ·{' '}
                                {riderBikes[0].bike_ratio}
                              </div>
                            ) : (
                              <div className="account-note">
                                {t('common.noBikeSaved')}
                              </div>
                            )}
                          </label>
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              startRun(manifest.id, selectedBike?.id || null)
                            }
                          >
                  ...
                  {resolvedShareCode && (
                    <div className="manifest-side-stack">
                      <div className="challenge-rivalry-card result-bridge-card">
                        ...
                        <strong className="share-code-value">
                          {resolvedShareCode}
                        </strong>
                        <em>{t('share.ready')}</em>
                      </div>
                  ...
                  <section
                    className="manifest-checkpoint-section"
                    id="manifest-checkpoints"
                  >
                    <div className="challenge-card-head checkpoint-section-head">
                      <div>
                        <div className="manifest-subtitle checkpoint-section-title">
                          {t('alleycat.checkpoints')}
                        </div>
                        <div className="challenge-card-copy checkpoint-section-copy">
                          {t('alleycat.result.checkpointsBody')}
                        </div>
                      </div>
                    </div>
                    <div className="checkpoint-list">
                      {orderedCheckpoints.map((checkpoint, index) => {
                        const isDone = completedCheckpointIds.includes(
                          checkpoint.id,
                        );
                        const isNext = !isDone && completedCount === index;
                        const showHint = Boolean(
                          hintUnlocked[checkpoint.id] || isDone,
                        );
                        const helpReady = Boolean(
                          isNext && nearCheckpointId === checkpoint.id,
                        );
                        const proof = checkpointProofs.find(
                          entry => entry.checkpoint_id === checkpoint.id,
                        );
                        const completionIndex =
                          completionOrderMap[checkpoint.id];
                        const actualCheckpointSeconds = riderCheckpointSeconds(
                          checkpoint.id,
                        );
                        const ghostCheckpointSeconds =
                          typeof completionIndex === 'number'
                        ...
                        const showGhostCheckpointRing = Boolean(
                          ...
                          ghostCheckpointSeconds !== null,
                        );
                        const ghostCheckpointProgress =
                          showGhostCheckpointRing &&
                          riderElapsedSeconds !== null &&
                          ghostCheckpointSeconds
                            ? Math.max(
                                0,
                                Math.min(
                                  100,
                        ...
                        return (
                          <div
                            key={checkpoint.id}
                            className={`checkpoint-card ${isDone ? 'done' : isNext ? 'up-next' : ''} ${proof ? 'proofed' : ''}`}
                          >
                            <div className="checkpoint-header">
                              <div className="checkpoint-meta">
                                <span className="checkpoint-number-pill">
                                  CP {index + 1}
                                </span>
                                <span className="checkpoint-district">
                                  {checkpoint.district}
                                </span>
                              </div>
                              <div className="checkpoint-header-tools">
                                {isDone ? (
                                  <div
                                    className="checkpoint-state-mark"
                                    aria-label={t(
                                      'alleycat.result.checkpointDone',
                                    )}
                                  >
                                    <Check size={16} />
                                  </div>
                                ...
                                {showGhostCheckpointRing ? (
                                  <div
                                    ...
                                    style={{
                                      ['--ghost-ring-progress' as any]: `${ghostCheckpointProgress}%`,
                                    }}
                                  ...
                                  >
                                    ...
                                    <span>
                                      {formatMinutes(ghostCheckpointSeconds)}
                                    </span>
                                  </div>
                                ) : null}
                              </div>
                            ...
                            <div className="manifest-metrics checkpoint-split-grid">
                              ...
                              <div>
                                ...
                                <strong>
                                  {actualCheckpointSeconds !== null
                                    ? formatMinutes(actualCheckpointSeconds)
                                    : '--'}
                                </strong>
                              </div>
                            </div>
                            <div className="checkpoint-actions">
                              {hasActiveRun && isNext ? (
                                <>
                                  <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                      void unlockCheckpointHint(checkpoint)
                                    }
                                  >
                                    <LocateFixed size={14} />
                                  ...
                                  <button
                                    className="primary-button small checkpoint-tick-button"
                                    type="button"
                                    onClick={() => checkIn(checkpoint.id)}
                                  >
                                    <Check size={14} />
                                    <span>
                                      {t('alleycat.result.checkpointClear')}
                                    </span>
                                  </button>
                                </>
                              ) : null}
                              <div className="checkpoint-footer-row">
                                <span
                                  className={`checkpoint-state-text ${isDone ? 'done' : isNext ? 'next' : 'waiting'}`}
                                >
                                  {isDone
                                    ? t('alleycat.result.checkpointDone')
                                    : isNext
                                      ? t('alleycat.result.checkpointNext')
                                      : t('alleycat.result.checkpointWaiting')}
                                </span>
                                <div className="checkpoint-order-controls">
                                  <button
                                    type="button"
                                    className="ghost-button small checkpoint-order-button"
                                    onClick={() =>
                                      moveCheckpoint(checkpoint.id, 'up')
                                    }
                                    disabled={index === 0}
                                  >
                                    <ArrowUp size={14} />
                                  ...
                                  <button
                                    ...
                                    className="ghost-button small checkpoint-order-button"
                                    onClick={() =>
                                      moveCheckpoint(checkpoint.id, 'down')
                                    }
                                    disabled={
                                      index === orderedCheckpoints.length - 1
                                    }
                                  >
                                    <ArrowDown size={14} />
                                  </button>
                            ...
                            {isDone ? (
                              ...
                              checkpoint.challenge.type !== 'photo' ? (
                                <div className="proof-panel checkpoint-proof-panel">
                                  <div className="proof-callout">
                                    <strong>
                                      {proof
                                        ? t('alleycat.result.proofReady')
                                        : t(
                                            `alleycat.challenge.${checkpoint.challenge.type}`,
                                          )}
                                    </strong>
                                    <span>
                                      {proof
                                        ? t('alleycat.result.proofReadyBody')
                                  ...
                                  {!proof && (
                                    <div className="checkpoint-actions checkpoint-proof-actions">
                                      <input
                                          ...
                                          if (
                                            ...
                                            void submitAnswer(
                                              ...
                                              if (result === 'correct') {
                                                ...
                                                setAnswerWrongCount(prev => ({
                                                  ...prev,
                                                  [checkpoint.id]:
                                                    (prev[checkpoint.id] || 0) +
                                                    1,
                                                }));
                                              }
                                            });
                                        ...
                                        placeholder={t(
                                          'alleycat.challenge.answerPlaceholder',
                                        )}
                                        disabled={Boolean(
                                          isUploading?.[checkpoint.id],
                                        )}
                                        autoComplete="off"
                                      />
                                      <button
                                      ...
                                      >
                                        {isUploading?.[checkpoint.id]
                                          ? t('alleycat.result.proofSending')
                                          : t('alleycat.challenge.submit')}
                                      </button>
                                    </div>
                                  )}
                                  {(answerWrongCount[checkpoint.id] || 0) > 0 &&
                                    !proof && (
                                      <div className="proof-callout error">
                                        <span>
                                          {t('alleycat.challenge.wrong')}
                                        </span>
                                      </div>
                                    )}
                                </div>
                                ...
                                <div className="proof-panel checkpoint-proof-panel">
                                  <div className="proof-callout">
                                    <strong>
                                      {proof
                                        ? t('alleycat.result.proofReady')
                                        : t('alleycat.result.proofNeeded')}
                                    </strong>
                                    <span>
                                      {proof
                                        ? t('alleycat.result.proofReadyBody')
                                        : t('alleycat.result.proofNeededBody')}
                                    </span>
                                  </div>
                                  <div className="checkpoint-actions checkpoint-proof-actions">
                                    <label className="ghost-button small checkpoint-proof-picker">
                                      <ImagePlus size={14} />
                                      <span>
                                        {proofFiles[checkpoint.id]?.name ||
                                          t('alleycat.result.pickProof')}
                                      </span>
                                      <input
                                        type="file"
                                        accept="image/*"
                                    ...
                                    <button
                                      ...
                                      type="button"
                                      disabled={
                                        Boolean(isUploading?.[checkpoint.id]) ||
                                        (!proofFiles[checkpoint.id] && !proof)
                                      }
                                      onClick={() => {
                                        if (
                                          !proof &&
                                          proofFiles[checkpoint.id]
                                        ) {
                                          void uploadProof(
                                            checkpoint,
                                            proofFiles[checkpoint.id],
                                    ...
                                    >
                                      {proof
                                        ? t('alleycat.result.proofSent')
                                        : isUploading?.[checkpoint.id]
                                          ? t('alleycat.result.proofSending')
                                          : t('alleycat.result.sendProof')}
                                    </button>
                                  </div>
                                </div>
                              )
                    ...
                    <section className="ghost-preview-panel">
                      <div className="ghost-preview-card">
                        <button
                          ...
                          type="button"
                          onClick={() => setShowGhostPanel(open => !open)}
                        >
                          <div>
                            <div className="manifest-subtitle">
                              {t('alleycat.result.ghostToggle')}
                            </div>
                          ...
                          <>
                            <div className="ghost-preview-shell">
                              <div className="ghost-preview-head">
                                <div>
                                  <div className="manifest-title">
                                    {manifest.ghost_label ||
                                      t('alleycat.run.ghost')}
                                  </div>
                              ...
                              <div className="manifest-metrics ghost-preview-metrics">
                                <div>
                                  ...
                                  <strong>
                                    {riderElapsedSeconds !== null
                                      ? formatMinutes(riderElapsedSeconds)
                                      : '--'}
                                  </strong>
                                </div>
                                <div>
                                  <span>
                                    {t('alleycat.result.nextGhostCall')}
                                  </span>
                                  <strong>
                                    {hasActiveRun || run?.status === 'finished'
                              ...
                              <div className="ghost-preview-track">
                                <div className="ghost-preview-line">
                                  <div
                                    className="ghost-preview-marker ghost"
                                    style={{left: `${ghostProgress}%`}}
                                  />
                                  <div
                                    className="ghost-preview-marker rider"
                              ...
                              <div className="ghost-split-grid">
                                {orderedCheckpoints.map((checkpoint, index) => {
                                  ...
                                  const splitActual = riderCheckpointSeconds(
                                    checkpoint.id,
                                  );
                                  return (
                                    <div
                                      key={checkpoint.id}
                                      className={`ghost-split-card ${done ? 'done' : next ? 'next' : ''}`}
                                    >
                                      <div className="ghost-split-meta">
                                        <span>{`CP ${index + 1}`}</span>
                                        <span
                                          className={`ghost-split-status ${done ? 'done' : next ? 'next' : ''}`}
                                        >
                                          {done
                                            ? t(
                                                'alleycat.result.ghostNoticeDone',
                                              )
                                            : next
                                              ? t(
                                                  'alleycat.result.ghostNoticeLive',
                                                )
                                              : t(
                                      ...
                                      <div className="ghost-split-name">
                                        {checkpoint.name}
                                      </div>
                                      <div className="manifest-metrics">
                                        <div>
                                          ...
                                          <strong>
                                            {hasActiveRun ||
                                            run?.status === 'finished'
                                              ? formatMinutes(splitTarget)
                                              : '--'}
                                          </strong>
                                        </div>
                                        <div>
                                          <span>
                                            {t('alleycat.run.yourTime')}
                                          </span>
                                          <strong>
                                            {splitActual !== null
                                              ? formatMinutes(splitActual)
                                              : '--'}
                                          </strong>
                                        </div>
                                      </div>
                  ...
                  <div className="route-actions manifest-result-actions">
                    ...
                    {hasActiveRun && !allCheckpointsCleared ? (
                      <button
                        className="ghost-button manifest-abandon-button"
                        type="button"
                        onClick={() => abandonRun()}
                      >
                  ...
                  {challenge && (
                    <section
                      className="challenge-board-shell"
                      id="challenge-board"
                    >
                      <div className="challenge-board-header">
                        <div>
                          <div className="section-label">
                            {t('alleycat.run.challengeBoard')}
                          </div>
                          <div className="result-title">
                            {t('alleycat.run.sharedStandings')}
                          </div>
                        </div>
                        <div className="challenge-board-code">
                          <strong className="challenge-board-code-value">
                            {challenge.code}
                          </strong>
                          {challengeSummary?.status ? (
                            <span
                              className={`challenge-board-status ${challengeSummary.status}`}
                            >
                              {challengeSummary.status}
                            </span>
                          ) : null}
                      ...
                      <div className="challenge-board-grid">
                        <div className="challenge-overview-card">
                          <div className="challenge-summary-copy">
                            <strong>
                              {challengeSummary?.winner_name
                                ? t('alleycat.board.winnerNow', {
                                    name: challengeSummary.winner_name,
                                  })
                                : t('alleycat.board.pending')}
                            </strong>
                            <span>
                              {challengeSummary?.rivalry ||
                                t('alleycat.board.pendingBody')}
                            </span>
                          </div>
                          <div className="challenge-stats-grid">
                            ...
                            <div className="challenge-stat">
                              ...
                              <strong>
                                {boardLeader?.best_seconds
                                  ? formatMinutes(boardLeader.best_seconds)
                                  : '--'}
                              </strong>
                            </div>
                          </div>
                        ...
                        <div className="challenge-leaderboard-card">
                          <div className="challenge-card-head">
                            <div>
                              <div className="manifest-subtitle">
                                {t('alleycat.board.title')}
                              </div>
                              <div className="challenge-card-copy">
                                {t('alleycat.board.subtitle')}
                              </div>
                            </div>
                          </div>
                            ...
                            {leaderboard.map((entry, index) => (
                              ...
                              >
                                ...
                                <div className="leaderboard-body">
                                  ...
                                  <span>
                                    {entry.best_seconds
                                      ? formatMinutes(entry.best_seconds)
                                      : t('alleycat.board.waiting')}
                                  </span>
                                </div>
                              </div>
                            ))}
        ...
        >
          ...
          >
            <div className="modal-header">
              <div className="modal-title" id="share-code-modal-title">
                {t('alleycat.haveCode')}
              </div>
            </div>
